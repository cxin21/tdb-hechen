...
export function registerChatMemoryRoutes(api: Hono, deps: PanelDeps): void {
        ...
        return respondEnvelope(
          ...
          okEnvelope(c, {
            ...
            items: (data.items ?? []).map((r) => ({
              ...
              tags: r.tags ?? [],
              refs: [],
              // 灵魂字段透传（时空网格 / 记忆图 UI 依赖；core atomic/query 已返回）
  ...
  // ==========================================================================
  // POST /chat-memory/health —— 记忆健康条（DS-PANEL-UI-WIKI-SOURCE-001 §2.3）
  //
  // 网关 GET /health 的 memory 子对象透传（T15 现成：vectorCoverage / embedding /
  // degradedSince）。上游 GET /health 是网关唯一免鉴权探活端点（MemoryCore
  // server.ts 明示豁免），Panel 侧仍要求有效 caller（auth/verify），与文件内其它
  // 路由的门控口径一致。
  // POST→GET 翻译照 settings.ts proxy-pricing 既有先例；网关 /health 返回原生
  // JSON（非信封），缺 memory 子对象按 502 如实失败（不造默认值，防静默降级）。
  // ==========================================================================
  api.post("/chat-memory/health", validatePanelMetaHeaders(deps), async (c) => {
    const ctx = buildCtx(c);
    const meUserId = await resolveCallerUserId(deps, ctx);
    if (!meUserId) return respondControlError(c, 401, "INVALID_USER_KEY");
    ...
    try {
      ...
      const resp = await fetch(`${coreBase}/health`, {
        method: "GET",
        headers,
        signal: ctrl.signal,
      });
      ...
      if (!resp.ok || !memory || typeof memory !== "object") {
        return respondControlError(c, 502, "HEALTH_UPSTREAM_INVALID");
      }
      return respondEnvelope(c, okEnvelope(c, { memory }));
    } catch (err) {
  ...
  // ==========================================================================
  // POST /chat-memory/archive/list  body: { block_id, limit?, offset? }
  //
  // 归档桶列表透传（DS-PANEL-UI-WIKI-SOURCE-001 §2.2，Task 4）→
  // 网关 /v3/atomic/archive/list。读权限与 /chat-memory/layer 同一套 ACL
  // （owner / team-shared / borrowed 任一）。
  // SEC-1（2026-09-11）后网关按请求 isolation 三元组过滤归档（本路由透传的
  // team/agent/user idFields 即过滤键），列表只含该 block 归属的归档；出参
  // additive 带回 team_id/user_id/agent_id（前端归属徽标消费），透传形状零改动。
  // 恢复（archive/restore）同样在网关侧做属主校验：跨租户 restore → 404。
  // ==========================================================================
  api.post(
    "/chat-memory/archive/list",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
      ...
      const parsed = parseChatMemoryAssetId(blockId);
      // 自建 UserAsset 无关联 agent → 无归档桶可查，返空（与 layer 语义一致）
      if (!parsed)
  ...
  // ==========================================================================
  // POST /chat-memory/archive/restore  body: { block_id, id }
  //
  // 归档恢复透传 → 网关 /v3/atomic/archive/restore（{id}；404 = 归档中不存在）。
  // 恢复是数据面 mutation：Owner-only（与 layer-delete / clear 同口径 —— 读可以
  // 借入，但恢复别人归档的记忆越权，借入方亦不得代恢复）。
  // ==========================================================================
  api.post(
    "/chat-memory/archive/restore",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
      ...
      if (!blockId) return respondControlError(c, 400, "MISSING_BLOCK_ID");
      if (!recordId) return respondControlError(c, 400, "MISSING_ID");
  ...
  // ==========================================================================
  // 价值锚编辑面板（DS-PANEL-UI-WIKI-SOURCE-001 §2.4，Task 5）
  //
  // S1（values upsert|delete）+ C2（derive）的 Panel BFF 化，列表复用
  // /v3/core-memory/read 的 values 数组（丢 slots）。租户 = block_id 反解的
  // (team, agent, asset.owner)（v2-router coreTenantFromIsolation 同形）。
  // 权限：读 authorizeChatMemoryRead；写（upsert/delete/derive）Owner-only，
  // 与 layer-delete / clear 同口径。
  //
  // 微调优先（三方语义 1）不变量在后端保证：upsert 显式 valence = 用户微调
  // （store clamp -1|0|1）；derive 只判 NULL 行（写回带 `valence IS NULL` 守卫，
  // apply-after-success），永不覆盖非 NULL —— 本组路由只做形状透传，不发明语义。
  // ==========================================================================

  /** 归一化 value_id（对齐内核 normalizeValueId：slug 风格小写）。 */
  function normalizeValueIdLocal(raw: string): string {
    return raw.trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9_-]/g, "");
  }

  // POST /chat-memory/values/list  body: { block_id }
  api.post(
    "/chat-memory/values/list",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
      ...
      const cred = toKernelCredentials(ctx, { timeoutMs: 15_000 });
      // GROW：可选 include_retired 透传（Panel 退休区；缺省不携带 = 网关默认 active-only，
      // 旧前端行为不变）。origin/pinned/state 字段由网关出参原样透传，本层不覆写。
      const includeRetired = body?.include_retired === true;
      const env = await deps.kernelHttp.postEnvelope<{
        slots?: unknown;
        values?: unknown[];
      }>(
  ...
  api.post(
    ...
    async (c) => {
      ...
      const body = await readJson(c);
      const blockId = requiredBlockId(body);
      const rawValueId = typeof body?.value_id === "string" ? body.value_id : "";
      const label = typeof body?.label === "string" ? body.label.trim() : "";
      const weight = typeof body?.weight === "number" && Number.isFinite(body.weight) ? body.weight : null;
      if (!blockId) return respondControlError(c, 400, "MISSING_BLOCK_ID");
      const valueId = normalizeValueIdLocal(rawValueId);
      if (!valueId) return respondControlError(c, 400, "MISSING_VALUE_ID");
      if (!label) return respondControlError(c, 400, "MISSING_LABEL");
      if (weight === null) return respondControlError(c, 400, "INVALID_WEIGHT");
      // valence 可选：显式传入（微调）必须是有穷数字（store clamp 三值）；
      // 未传 = plain upsert（新建落 NULL 待 LLM 判，冲突不触碰原值）。
      let valence: number | undefined;
      if (body?.valence !== undefined) {
        if (typeof body.valence !== "number" || !Number.isFinite(body.valence)) {
          return respondControlError(c, 400, "INVALID_VALENCE");
        }
  ...
  // POST /chat-memory/values/delete  body: { block_id, value_id }
  api.post(
    "/chat-memory/values/delete",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
  ...
  // POST /chat-memory/values/pin  body: { block_id, value_id, pinned }
  // GROW（钉住/取消钉住）：pinned=1 永不被自生长挤出。透传形状，不发明语义。
  api.post(
    "/chat-memory/values/pin",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
      ...
      const valueId = normalizeValueIdLocal(rawValueId);
      if (!valueId) return respondControlError(c, 400, "MISSING_VALUE_ID");
      if (typeof body?.pinned !== "boolean") return respondControlError(c, 400, "INVALID_PINNED");
  ...
  // POST /chat-memory/values/retire  body: { block_id, value_id }
  // GROW（手动退休）：state → 'retired'（退休区可见，可恢复/可钉住；可逆）。
  api.post(
    "/chat-memory/values/retire",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
  ...
  // POST /chat-memory/values/restore  body: { block_id, value_id }
  // GROW（恢复）：retired → active（退休区[恢复]；vetoed 由网关拒绝）。
  api.post(
    "/chat-memory/values/restore",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
  ...
  // POST /chat-memory/values/derive  body: { block_id }（C2 重新总结方向，无业务覆写）
  api.post(
    "/chat-memory/values/derive",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
  ...
  // POST /chat-memory/values/discover  body: { block_id }（Task DISC 价值锚发现，提议制）
  // 纯只读透传：网关侧 LLM 蒸馏提案（只提议不落库，K1 信任边界），Panel 不携带任何
  // 提案字段。LLM 蒸馏耗时高于 derive → 上游超时放宽到 60s。
  api.post(
    "/chat-memory/values/discover",
    validatePanelMetaHeaders(deps),
    async (c) => {
      const ctx = buildCtx(c);
      const body = await readJson(c);
      ...
      const env = await deps.kernelHttp.postEnvelope<{
        proposals?: Array<{ label: string; rationale: string; evidenceCount: number; suggestedWeight: number }>;
        sampleSize?: number;
      }>(
        "/v3/core-memory/values/discover",
        {
          team_id: parsed.teamId,
          agent_id: parsed.agentId,
          user_id: asset.owner_user_id,
  ...
  // ==========================================================================
  // POST /chat-memory/neighbors
  //   body: { block_id, id /* 某条 L1 记录主键 */, types?, maxHop?, maxN? }
  //
  // 记忆图（G）：取某条记忆的邻接（l1_links 边），供前端"记忆图视图"渲染。
  // 权限与 /chat-memory/search 一致（读权限）。代理内核 /v3/atomic/neighbors。
  // ==========================================================================
  api.post("/chat-memory/neighbors", validatePanelMetaHeaders(deps), async (c) => {
    const ctx = buildCtx(c);
    const body = await readJson(c);
    const blockId = requiredBlockId(body);
    const memId = typeof body?.id === "string" ? body.id.trim() : "";
    if (!blockId) return respondControlError(c, 400, "MISSING_BLOCK_ID");
    if (!memId) return respondControlError(c, 400, "MISSING_MEMORY_ID");

    const parsed = parseChatMemoryAssetId(blockId);
    if (!parsed)
      return respondEnvelope(c, okEnvelope(c, { id: memId, neighbors: [] }));
    ...
    const cred = toKernelCredentials(ctx, { timeoutMs: 20_000 });
    const types = Array.isArray(body?.types) ? (body.types as string[]) : undefined;
    const maxHop = typeof body?.maxHop === "number" ? Math.min(3, body.maxHop) : 1;
    const maxN = typeof body?.maxN === "number" ? body.maxN : 20;

    try {
      const env = await deps.kernelHttp.postEnvelope<{ neighbors?: unknown[] }>(
        "/v3/atomic/neighbors",
        { ...idFields, id: memId, types, maxHop, maxN },
        cred,
      );
      if (env.code !== 0) return respondEnvelope(c, env);
      const data = (env.data as { neighbors?: unknown[] } | null) ?? { neighbors: [] };
      return respondEnvelope(c, okEnvelope(c, { id: memId, neighbors: data.neighbors ?? [] }));
    } catch {
      return respondControlError(c, 500, "NEIGHBORS_FAILED");
    }
  });

  // ==========================================================================
  // POST /chat-memory/path
  //   body: { block_id, startId, endId, maxHop?, types? }
  //
  // 关联链（U-A3，S2 RelatedSection）：两节点 BFS 最短路径，代理内核
  // /v3/atomic/path（C6 生产路由；store 未实现 getPath → path: null 如实透出）。
  // 权限与 /chat-memory/neighbors 一致（读权限）。maxHop clamp [1,3]（与内核
  // handleAtomicPath 护栏一致，面板层同 clamp 防呆）。
  // ==========================================================================
  api.post("/chat-memory/path", validatePanelMetaHeaders(deps), async (c) => {
    const ctx = buildCtx(c);
    const body = await readJson(c);
    const blockId = requiredBlockId(body);
    const startId = typeof body?.startId === "string" ? body.startId.trim() : "";
    const endId = typeof body?.endId === "string" ? body.endId.trim() : "";
    if (!blockId) return respondControlError(c, 400, "MISSING_BLOCK_ID");
    if (!startId) return respondControlError(c, 400, "MISSING_START_ID");
    if (!endId) return respondControlError(c, 400, "MISSING_END_ID");

    const parsed = parseChatMemoryAssetId(blockId);
    if (!parsed)
      return respondEnvelope(
        c,
        okEnvelope(c, { startId, endId, path: null }),
      );
    ...
    const cred = toKernelCredentials(ctx, { timeoutMs: 20_000 });
    const maxHop =
      typeof body?.maxHop === "number" && Number.isFinite(body.maxHop)
        ? Math.max(1, Math.min(Math.floor(body.maxHop), 3))
        : 3;
    const types = Array.isArray(body?.types) ? (body.types as string[]) : undefined;
    ...
    try {
      const env = await deps.kernelHttp.postEnvelope<{
        ...
        "/v3/atomic/path",
        { ...idFields, startId, endId, maxHop, ...(types ? { types } : {}) },
        cred,
      );
      if (env.code !== 0) return respondEnvelope(c, env);
      const data = (env.data as { path?: unknown } | null) ?? {};
      return respondEnvelope(
        c,
        okEnvelope(c, { startId, endId, path: data.path ?? null }),
      );
    } catch {
      return respondControlError(c, 500, "PATH_FAILED");
    }
  });
  ...
  api.post("/chat-memory/search", validatePanelMetaHeaders(deps), async (c) => {
    ...
    try {
      ...
      const items = (data.items ?? []).map((r) => ({
        ...
        // 灵魂字段 + metadata 透传（U-A2 S1 记忆卡片增强的数据前提）：
        // 与 /chat-memory/layer L1 同形（896ee5d），搜索召回的卡片才能与
        // 浏览卡片一致显示灵魂区；字段不在场时为 undefined，UI 宁缺毋滥。
        occurred_at: r.occurred_at,
        valid_start: r.valid_start,
        valid_end: r.valid_end,
        certainty: r.certainty,
...
/** /chat-memory/health 上游探活超时（状态条不应拖慢页面）。 */
