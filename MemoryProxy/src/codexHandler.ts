...
// upstream.agents.codex > 全局。缓存为 per-module 单例(共享底层 ProxyStorage);
// 路由写后跨模块显式 invalidate 不可达,成员改配置最迟按 spec §3.3 ≤30s TTL
// 收敛(已接受的取舍)。
...
async function forwardToUpstream(
  ...
  // url/apiKey/body.model 全取成员配置,优先级高于 upstream.agents.codex。
  ...
  const upstreamBase = memberUpstream?.url || agentUpstreamEntry?.url || config.upstream.url;
  ...
  // 否则透传客户端 Bearer。
  if (memberUpstream) {
    upstreamHeaders["authorization"] = `Bearer ${memberUpstream.apiKey}`;
    delete upstreamHeaders["x-api-key"];
  } else if (agentUpstreamEntry) {
