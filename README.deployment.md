...
# 推荐 — 网关鉴权（P2-T13 起，鉴权必填）
# 不配置时网关每次启动会生成一把随机临时密钥（本进程有效，重启更换）并
# 在启动日志 loud 打印 —— 所有调用方（MemoryProxy / Panel / SDK / 探针脚本）
# 必须带 `Authorization: Bearer <key>`，且与网关同一把 key，否则一律 401。
export TDAI_GATEWAY_API_KEY="your-shared-key"
# 跨进程组件（MemoryProxy / MemoryPanel）读取 env TDAI_GATEWAY_APIKEY 透传同一把 key
```
...
server:
  port: 8420
  host: "127.0.0.1"
  # P2-T13 鉴权必填：不配则每次启动生成临时密钥（重启更换），跨进程调用方须配同一把 key
  # apiKey: "your-key"  # 不配则每次启动生成临时密钥
