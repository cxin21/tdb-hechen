...
export function WikiActions({
  ...
  /** 当前这条 wiki 是否有源管理操作在执行（编辑保存/测试/暂停/重拉） */
  sourceBusy?: boolean;
  ...
  /** git 源卡片操作区（DS-PANEL-UI-WIKI-SOURCE-001 §1.4）：编辑 / 暂停启用 / 重新拉取 */
  onEdit?: (source: WikiDetail) => void;
  onToggleEnabled?: (source: WikiDetail) => void;
  onRefetch?: (wikiId: string) => void;
}) {
  const { t } = useTranslation();
  const isGit = source.source_type === 'git';
  const enabled = source.enabled ?? true;
  return (
    <div className="_asset-wiki-actions" onClick={(event) => event.stopPropagation()}>
      {isGit && onEdit && (
        <Button type="weak" disabled={sourceBusy} onClick={() => onEdit(source)}>
          {t('wiki.action.edit')}
        </Button>
      )}
      {isGit && onRefetch && (
        <Button
          type={source.stale ? 'primary' : 'weak'}
          disabled={ingestBusy || sourceBusy}
          tooltip={
            source.stale
              ? source.sync_error
                ? t('wiki.action.refetch.staleWithError')
                : t('wiki.action.refetch.stale')
              : t('wiki.action.refetch.hint')
          }
          onClick={() => onRefetch(source.wiki_id)}
        >
          {t('wiki.action.refetch')}
        </Button>
      )}
      {isGit && onToggleEnabled && (
        <Button type="weak" disabled={sourceBusy} onClick={() => onToggleEnabled(source)}>
          {enabled ? t('wiki.action.pause') : t('wiki.action.resume')}
        </Button>
      )}
