...
import { setTdaiGatewayApiKey } from "./tdai/bearer-token.js";
...
import { applyStoredCreditPricing } from "./routes/credit-pricing.js";
import { flushPendingWrites, pendingWriteCount } from "./tdai/pending-writes.js";

const overrides = parseArgv(process.argv);
const config = buildConfig(overrides);

// ── I-1（P2-T11+13 复核修补）：skill/knowledge/meta 三链路 Bearer token 兜底 ──
// 把 yaml `tdai.apiKey` 注入共享解析器（resolveBearerToken 的第 2 级），使这三条
// 原先只读各自 serviceToken 的链路也纳入 `TDAI_GATEWAY_APIKEY` 透传范围，
// 鉴权翻转后不再静默 401 断供。env 优先级最高，无需在此处理。
setTdaiGatewayApiKey(config.tdai.apiKey);
...
// ── spec §9：重启后恢复管理端价目表 override（storage 有则覆盖 yaml 底表）────
await applyStoredCreditPricing(config);
