...
export const knowledgeWiki = sqliteTable(
  "knowledge_wiki",
  {
    ...
    // git-source columns: branch defaults to 'main' at service layer; regex filters null = no restriction.
    branch: text("branch"),
    pathInclude: text("path_include"),
    pathExclude: text("path_exclude"),
    ownerUserId: text("owner_user_id"),
    userId: text("user_id"),
    agentId: text("agent_id"),
    taskId: text("task_id"),
    ...
    serviceUrl: text("service_url"),
    summary: text("summary"),
    // source management (DS-PANEL-UI-WIKI-SOURCE-001): 1/0 in DB, mapped to boolean in store.
