...
import { knowledgeApi, pollWikiStatus } from '@/lib/api/knowledge-api';
import { tea } from '@/lib/tea-bridge';
import { WIKI_ALLOWED_FILE_RE, TYPE_COLORS, TYPE_COLOR_FALLBACK, formatShortTime, type DetailTab } from '../constants/wiki-constants';
...
export function WikiDetailView({ store }: { store: WikiSourcesStore }) {
  ...
  const isGitSource = source?.source_type === 'git';
  const [syncing, setSyncing] = useState(false);

  /** 手动同步 Git 源：触发 sync → 后台轮询到 ready/failed 再刷新并提示（fire-and-forget，不设固定短超时）。 */
  const handleSync = async () => {
    if (!selectedWikiId || !isGitSource) return;
    setSyncing(true);
    try {
      await knowledgeApi.wiki.sync(selectedWikiId);
      tea.notify.success(t('wiki.notify.synced'));
      fetchSources();
    } catch (e: unknown) {
      // 409 busy 表示已有同步任务在跑，视为正常（已开始），不当作失败
      ...
      tea.notify.error(e instanceof Error ? e.message : String(e));
      setSyncing(false);
      return;
    }
    // 后台轮询同步状态到 ready/failed。git 源通常要拉取 + 长时间 ingest，
    // 因此用较长窗口（10 分钟），且失败（status=failed）才提示失败；
    // 超时只提示"仍在同步"，不误报失败。完成后刷新列表。
    pollWikiStatus(selectedWikiId, 300, 2000)
      .then((detail) => {
        if (detail.status === 'failed') {
          tea.notify.error(t('wiki.notify.syncFailed'));
        } else {
          tea.notify.success(t('wiki.notify.syncComplete'));
        }
        fetchSources();
      })
      .catch(() => {
        // 超时：后台可能仍在拉取/ingest，不误报失败，仅提示去刷新查看
        tea.notify.info(t('wiki.notify.syncRunning'));
        fetchSources();
      })
    ...
    <div className="_wiki-detail-root">
      <Card>
        <Card.Body className="_wiki-detail-header-body">
            ...
            <div className="_wiki-detail-header-actions">
              ...
              {isGitSource && (
                <Button
                  ...
                  disabled={syncing || ingestBusy}
                  loading={syncing}
                >
                  <RefreshIcon size={14} /> {syncing ? t('wiki.detail.syncing') : t('wiki.detail.sync')}
                </Button>
              )}
              <Button
                type="primary"
          ...
          {isGitSource && (
            <div
              className="_wiki-detail-source"
              style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 10 }}
            >
              <Text theme="label">
                {t('wiki.detail.source')}: {t('wiki.source.git')}
              </Text>
              <Text theme="weak" style={{ wordBreak: 'break-all', fontFamily: 'Consolas, Monaco, monospace' }}>
                {source.source_url ?? '—'}
                {source.branch ? ` @ ${source.branch}` : ''}
              </Text>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, color: 'var(--tea-color-text-tertiary)' }}>
                <span>
                  {t('wiki.detail.pathInclude')}: {source.path_include ? `/${source.path_include}/` : t('wiki.detail.pathInclude.default')}
                </span>
                {source.path_exclude && (
                  <span>
                    {t('wiki.detail.pathExclude')}: /{source.path_exclude}/
                  </span>
                )}
              </div>
              <Text theme="weak">
                {t('wiki.detail.lastSyncAt')}: {formatShortTime(source.last_sync_at)}
              </Text>
            </div>
          )}
        </Card.Body>
