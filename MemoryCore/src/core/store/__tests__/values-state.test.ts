...
describe("GROW anchor_growth_state kv 持久化", () => {
  it("set → get 回读；重开库仍在（重启不失忆）；get 未初始化 → null 基线", () => {
    ...
    if (store.isDegraded()) throw new Error(`临时库初始化降级（环境问题）：${initRes.reason}`);
    try {
      expect(store.getAnchorGrowthState()).toEqual({ lastDiscoveryAt: null, lastCorpusCount: null });
      store.setAnchorGrowthState({ lastDiscoveryAt: "2026-09-10T08:00:00.000Z", lastCorpusCount: 42 });
      expect(store.getAnchorGrowthState()).toEqual({ lastDiscoveryAt: "2026-09-10T08:00:00.000Z", lastCorpusCount: 42 });
    } finally {
    ...
    store2.init();
    try {
      expect(store2.getAnchorGrowthState()).toEqual({ lastDiscoveryAt: "2026-09-10T08:00:00.000Z", lastCorpusCount: 42 });
