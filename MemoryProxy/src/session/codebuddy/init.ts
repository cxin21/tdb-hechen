...
export interface SessionInitResult {
  ...
  /** session-reset 命令相关：透传 reset 语义给 handler（见各 handler 消费处）。 */
  resetFlow?: boolean;
...
function applyArtifactsAndContext(
  ...
  sessionKey: string,
  config: SessionInitConfig,
  opts?: { stripForms?: boolean },
): MessageArr {
  // 曾在按 config.keepInitArtifacts 决定要不要 stripInitArtifacts；那个开关后来移除了。
  // 但对**单 agent auto-select 完成注册**这类场景，若把 proxy 注入的 fake form
  // （assistant ask_user_question + tool 回答"是,关联"）原样透传给上游模型，
  // 模型会误以为"用户要关联但未选定 agent"，从而续问 → 产出 id/name 为空的
  // tool_call → dsh 报 `Error: unknown tool ""` / upstream 400
  // `missing messages.tool_call_id`。因此当调用方显式声明 stripForms 时，
  // 在注入上下文前剥离这些 session-init 表单帧，只保留用户真实对话与注入结果。
  let base: MessageArr = messages;
  if (opts?.stripForms) {
    const stripped = stripInitArtifacts(messages as RawMessage[]) as MessageArr;
    if (stripped.length !== messages.length) {
      console.log(
        `[session-init:cb] session=${sessionKey} stripInitArtifacts: ${messages.length} → ${stripped.length} msgs`,
      );
      base = stripped;
    }
  }
  const injected = injectSessionContextWithToggles(base, agentDetail, taskDetail, config, sessionKey);
  if (injected !== base) {
    const finalRoles = (injected as unknown[]).map((m: any) => m.role);
    console.log(
      `[session-init:cb] session=${sessionKey} processed: ${base.length} msgs, ` +
...
export async function completeRegistration(
  ...
  const out = applyArtifactsAndContext(messages, agentDetail, taskDetail, compositeKey, config, { stripForms: true });
  return {
    intercepted: false,
    messages: out,
    sessionInfo,
...
async function handleSessionInitInner(
    ...
    if (!presetIdentity) {
      ...
      if (decision.kind === "preset") {
        ...
        if (pr.hadMismatch) {
          ...
          if (presetTeam) {
            // 只配了 team → 按 header team-only 分支渲染 agent 表单（弹 Agent 表单，只问缺失那一级；
            // codex/wb/dsh/opencode 拆 stage，CB 客户端保持老 pending_agent_task 一发同时问）。
            const nextStatus = (isCodexClient || agentSource === "workbuddy" || agentSource === "dsh" || agentSource === "opencode") ? "pending_agent_select" : "pending_agent_task";
            const nextStage: FormData["stage"] = (isCodexClient || agentSource === "workbuddy" || agentSource === "dsh" || agentSource === "opencode") ? "agent_select" : "agent_task";
            console.log(
              `[session-init:cb] session=${compositeKey} user=${userId} session_init preset team=${pr.teamId} → ${nextStatus}`,
            );
            await store.set(compositeKey, {
              status: nextStatus,
              keyId: sessionKey,
        ...
        // 其余情况 fall through → 弹 asset_confirm。
      }
  ...
  const out = applyArtifactsAndContext(messages, agent, task, sessionKey, config, { stripForms: true });
