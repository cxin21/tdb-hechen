      ...
      if (decision.kind === "preset") {
        ...
        if (pr.hadMismatch) {
          ...
          if (presetTeam) {
            ...
            // 只配 default_team_id、缺 default_agent_id → 始终弹 Agent 单级表单（不因团队
            // 有 0 或 1 个 agent 而自动注册 / 绕过）。与 CB state machine 行为对齐。
            await store.set(compositeKey, {
              ...seedState,
              status: "pending_agent_select",
              selectedTeamId: pr.teamId,
            ...
            console.log(
              `[session-init:cc] session=${compositeKey} user=${userId} session_init preset team=${pr.teamId} → pending_agent_select (agent missing)`,
            );
            const fd: FormData = {
              teams,
              stage: "agent_select",
