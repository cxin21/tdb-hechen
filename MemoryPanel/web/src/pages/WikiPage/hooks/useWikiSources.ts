...
import { knowledgeApi, wikiProgressPercent, wikiStageLabel, type GraphData, type WikiDetail, type WikiPage, type WikiSourceProbeResult } from '@/lib/api/knowledge-api';
...
export function useWikiSources() {
    ...
    await confirmThenRun(
      {
        ...
        description: t('wiki.confirm.delete.desc'),
  ...
  // --- 源管理（DS-PANEL-UI-WIKI-SOURCE-001 Task 2）---
  // 正在执行源管理操作（编辑保存/测试连接/暂停切换/重拉）的 wiki_id；用于按钮 loading/禁用。
  const [sourceBusyId, setSourceBusyId] = useState('');

  /** 保存源编辑（url/branch）。成功后后端置 stale，徽标保持到"重新拉取"重建成功（后端清除）。 */
  const handleUpdateSource = async (
    wikiId: string,
    patch: { source_url?: string; branch?: string },
  ): Promise<boolean> => {
    setSourceBusyId(wikiId);
    try {
      await knowledgeApi.wiki.updateSource(wikiId, patch);
      tea.notify.success(t('wiki.notify.sourceUpdated'));
      await fetchSources();
      return true;
    } catch (e: unknown) {
      tea.notify.error((e instanceof Error ? e.message : String(e)) || t('wiki.notify.sourceUpdateFailed'));
      return false;
    } finally {
      setSourceBusyId('');
    }
  ...
  /** 测试连接（ls-remote）。sourceUrl 缺省探测已存源；探测失败是 reachable=false，不是请求错误。 */
  const handleTestSource = async (
    wikiId: string,
    sourceUrl?: string,
  ): Promise<WikiSourceProbeResult | null> => {
    setSourceBusyId(wikiId);
    try {
      return await knowledgeApi.wiki.testSource(wikiId, sourceUrl);
    } catch (e: unknown) {
      tea.notify.error((e instanceof Error ? e.message : String(e)) || t('wiki.notify.sourceTestFailed'));
      return null;
    } finally {
      setSourceBusyId('');
    }
  ...
  /** 暂停/启用自动同步：enabled-only 补丁不触碰 stale、不影响已摄入页面。 */
  const handleToggleEnabled = async (source: WikiDetail): Promise<void> => {
    setSourceBusyId(source.wiki_id);
    try {
      await knowledgeApi.wiki.updateSource(source.wiki_id, { enabled: !(source.enabled ?? true) });
      tea.notify.success(
        source.enabled ?? true ? t('wiki.notify.enabledOff') : t('wiki.notify.enabledOn'),
      );
      await fetchSources();
    } catch (e: unknown) {
      tea.notify.error((e instanceof Error ? e.message : String(e)) || t('wiki.notify.toggleFailed'));
    } finally {
      setSourceBusyId('');
    }
  };

  /** 重新拉取（stale 源手动触发）：202 异步重建。stale 徽标保持到重建成功——后端清，前端不猜。 */
  const handleRefetch = async (wikiId: string): Promise<void> => {
    if (ingestBusy) {
      tea.notify.warning(t('wiki.ingest.warning'));
      return;
    }
    setSourceBusyId(wikiId);
    try {
      await knowledgeApi.wiki.refetch(wikiId);
      tea.notify.info(t('wiki.notify.refetchQueued'));
      // 刷新列表：wiki 转入 pending/processing 后既有轮询自动接管进度。
      await fetchSources();
    } catch (e: unknown) {
      tea.notify.error((e instanceof Error ? e.message : String(e)) || t('wiki.notify.refetchFailed'));
    } finally {
      setSourceBusyId('');
    }
  };
