...
import type {
  ...
  WikiListResult,
  WikiIngestResult,
  WikiSourceUpdatePatch,
  WikiSourceProbeResult,
  WikiRefetchResult,
  WikiGraphData,
  WikiSearchResult,
  BatchDeleteResult,
...
export class HttpKnowledgeClient implements KnowledgeClientPort {
  ...
  async wikiCreate(teamId: string, name: string, userId?: string, opts?: WikiCreateOptions): Promise<WikiDetail> {
  ...
  async wikiSync(wikiId: string): Promise<WikiIngestResult> {
    return this.post('/v3/wiki/sync', { wiki_id: wikiId });
  }
  ...
  // ── 源管理（DS-PANEL-UI-WIKI-SOURCE-001 §1.3）──
  // PATCH /wiki/:id/source：url/branch/enabled；stale 置位与清除全在 KS 侧。
  async wikiUpdateSource(wikiId: string, patch: WikiSourceUpdatePatch): Promise<WikiDetail> {
    return this.request('PATCH', `/v3/wiki/${wikiId}/source`, patch);
  }

  // POST /wiki/:id/test：ls-remote 探测；sourceUrl 覆盖时 KS 仍走完整校验。
  async wikiTestSource(wikiId: string, sourceUrl?: string): Promise<WikiSourceProbeResult> {
    return this.post(`/v3/wiki/${wikiId}/test`, sourceUrl ? { source_url: sourceUrl } : {});
  }

  // POST /wiki/:id/refetch：KS 返回 202 信封（resp.ok 成立），busy 409 由信封 code 映射为 CoreUpstreamError(409)。
  async wikiRefetch(wikiId: string): Promise<WikiRefetchResult> {
    return this.post(`/v3/wiki/${wikiId}/refetch`, {});
  }
