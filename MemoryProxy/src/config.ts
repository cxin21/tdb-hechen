...
export const DEFAULT_CONFIG: ProxyConfig = {
  ...
  tdai: {
    ...
    memory: {
      ...
      injectL2L3: false,
      l1Limit: 5,
      // R7 复审 Minor ③：globalTopK 由硬编码 5 配置化；缺省 5 = 现行为不变。
      l1GlobalTopK: 5,
      l2Limit: 3,
      timeoutMs: 3000,
    },
  },
  // K/L 当前感受（appraisal）默认关（零影响；yaml 里 appraisal.values 显式开启）
  appraisal: { enabled: false, firedThreshold: 0.4, values: [] },
  coreSkill: {
...
export function buildConfig(overrides: CliOverrides = {}): ProxyConfig {
  ...
  return {
    ...
    creditReport: {
      enabled: yaml.creditReport?.enabled ?? DEFAULT_CONFIG.creditReport.enabled,
  ...
  sessionInit: {
    ...
    tdai: {
      ...
      memory: {
        ...
        // R7 复审 Minor ③：L1 召回合并 top-K 配置化（缺省 5 = 现行为不变）。
        l1GlobalTopK: yaml.tdai?.memory?.l1GlobalTopK ?? DEFAULT_CONFIG.tdai.memory.l1GlobalTopK,
    ...
    // K/L 当前感受（appraisal）：显式解析 yaml.appraisal；缺省 enabled=false（零影响）
    appraisal: yaml.appraisal && typeof yaml.appraisal === "object" && Array.isArray((yaml.appraisal as { values?: unknown }).values)
      ? {
          enabled: (yaml.appraisal as { enabled?: boolean }).enabled === true,
          firedThreshold: typeof (yaml.appraisal as { firedThreshold?: unknown }).firedThreshold === "number"
            ? (yaml.appraisal as { firedThreshold: number }).firedThreshold
            : 0.4,
          values: ((yaml.appraisal as { values: Array<{ id?: unknown; label?: unknown; weight?: unknown }> }).values)
            .filter((v) => typeof v?.label === "string" && typeof v?.weight === "number")
            .map((v, i) => ({ id: typeof v.id === "string" ? v.id : `v${i}`, label: v.label as string, weight: v.weight as number })),
        }
      : { enabled: false, firedThreshold: 0.4, values: [] },
    ...
    // WikiRecallInjector 可选配置段（`config.wikiRecall`）。缺失 → 装配端走
    // defaultWikiRecallConfig（含 minInjectNormScore:0.6）。
    wikiRecall: yaml.wikiRecall
      ? {
          enabled: yaml.wikiRecall.enabled !== false,
          perWikiLimit: typeof yaml.wikiRecall.perWikiLimit === "number" ? yaml.wikiRecall.perWikiLimit : 8,
          globalTopK: typeof yaml.wikiRecall.globalTopK === "number" ? yaml.wikiRecall.globalTopK : 5,
          minScore: typeof yaml.wikiRecall.minScore === "number" ? yaml.wikiRecall.minScore : 0.02,
          hop: typeof yaml.wikiRecall.hop === "number" ? yaml.wikiRecall.hop : 0,
          decay: typeof yaml.wikiRecall.decay === "number" ? yaml.wikiRecall.decay : 0.5,
          maxTotalChars: typeof yaml.wikiRecall.maxTotalChars === "number" ? yaml.wikiRecall.maxTotalChars : 0,
          minInjectNormScore: typeof yaml.wikiRecall.minInjectNormScore === "number" ? yaml.wikiRecall.minInjectNormScore : 0.6,
          typeWeights: yaml.wikiRecall.typeWeights,
          minInjectAbsScore: typeof yaml.wikiRecall.minInjectAbsScore === "number" ? yaml.wikiRecall.minInjectAbsScore : undefined,
          domainRouter: yaml.wikiRecall.domainRouter,
        }
      : undefined,
    skillRuntime: {
      allowLlmWrite:
