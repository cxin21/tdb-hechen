...
export function migrate(_db: Db, raw: Database.Database): void {
  ...
  // wiki git-source columns (branch / path_include / path_exclude) — added to existing DBs.
  addColumnIfMissing(raw, "knowledge_wiki", "branch", "TEXT");
  addColumnIfMissing(raw, "knowledge_wiki", "path_include", "TEXT");
  addColumnIfMissing(raw, "knowledge_wiki", "path_exclude", "TEXT");
  // wiki source-management columns (DS-PANEL-UI-WIKI-SOURCE-001) — legacy rows default enabled.
  addColumnIfMissing(raw, "knowledge_wiki", "enabled", "INTEGER NOT NULL DEFAULT 1");
  addColumnIfMissing(raw, "knowledge_wiki", "stale", "INTEGER NOT NULL DEFAULT 0");
