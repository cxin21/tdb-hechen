...
export async function verifyUserKey(userKey: string, serviceId: string): Promise<VerifyUserResult> {
  ...
  try {
    // 拍板③ 补漏：auth 链路也要带服务级 Bearer —— core gateway 对全路由
    // （含 /v3/meta/auth/verify）强制 `Authorization: Bearer <server.apiKey>`，
    // 缺失会被网关中间件 401 秒拒（I-1 已补 skill/knowledge/meta 三链路，
    // 本链路当时被遗漏）。解析语义与三链路一致：env TDAI_GATEWAY_APIKEY >
    // yaml tdai.apiKey（bootstrap 注入）> 原 serviceToken（auth 无自有
    // token，传空串）。皆空时不带头，保持无鉴权网关场景的历史行为。
