...
// ========================= Session Init Defaults（meta/config/user/session_init）=========================
//
// 纯前端 UI 包装：读 / 写当前用户 session_init 模块配置。
// /api/v1/meta/config/user/get · set 已在后端 ALLOWED_PANEL_ACTIONS 放行，零后端改动。

/** session_init module 下当前用户的默认配置（键盘不进，全靠滑块 + 下拉配置）。 */
