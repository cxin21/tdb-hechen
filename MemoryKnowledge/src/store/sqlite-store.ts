...
export class SqliteKnowledgeStore implements IKnowledgeStore {
      ...
      try {
        this.db
          ...
          .values({
            ...
            pathInclude: input.path_include ?? null,
            pathExclude: input.path_exclude ?? null,
            ownerUserId: input.owner_user_id ?? null,
            userId: input.user_id ?? null,
            agentId: input.agent_id ?? null,
            taskId: input.task_id ?? null,
  ...
  updateWikiStatus(serviceId: string, wikiId: string, patch: WikiStatusPatch): void {
    ...
    if (patch.service_url !== undefined) set.serviceUrl = patch.service_url;
    if (patch.summary !== undefined) set.summary = patch.summary;
    if (patch.source_url !== undefined) set.sourceUrl = patch.source_url;
    if (patch.branch !== undefined) set.branch = patch.branch;
    if (patch.path_include !== undefined) set.pathInclude = patch.path_include;
    if (patch.path_exclude !== undefined) set.pathExclude = patch.path_exclude;
    if (patch.enabled !== undefined) set.enabled = patch.enabled ? 1 : 0;
    if (patch.stale !== undefined) set.stale = patch.stale ? 1 : 0;
    if (patch.version !== undefined) set.version = patch.version;
  ...
  /**
   * Sweep non-terminal (pending/processing) assets on restart.
   * Only assets that never synced successfully (last_sync_at IS NULL, no usable index on disk)
   * are marked failed; assets that previously synced (last_sync_at non-null) are recovered to
   * ready so auto-sync / serving keeps working instead of being stranded as failed forever.
   * @returns total rows marked failed (code + wiki combined).
   */
  markInterruptedAsFailed(reason = "interrupted by restart"): number {
    ...
    const a = this.db
      ...
      .where(and(sql`status IN ('pending','processing')`, isNull(knowledgeCodeGraph.lastSyncAt)))
      .run();
    // Previously-synced non-terminal assets -> back to ready (keep index usable; clear stuck internal state).
    this.db
      .update(knowledgeCodeGraph)
      .set({ status: "ready", internalStatus: null, syncError: null, updatedAt: ts })
      .where(and(sql`status IN ('pending','processing')`, sql`last_sync_at IS NOT NULL`))
      .run();
    const b = this.db
      .update(knowledgeWiki)
      .set({ status: "failed", syncError: reason, updatedAt: ts })
      .where(and(sql`status IN ('pending','processing')`, isNull(knowledgeWiki.lastSyncAt)))
      .run();
    this.db
      .update(knowledgeWiki)
      .set({ status: "ready", internalStatus: null, syncError: null, updatedAt: ts })
  ...
  private mapWikiRow(r: typeof knowledgeWiki.$inferSelect): WikiRow {
    return {
      ...
      branch: r.branch,
      path_include: r.pathInclude,
      path_exclude: r.pathExclude,
      owner_user_id: r.ownerUserId,
      user_id: r.userId,
      agent_id: r.agentId,
      task_id: r.taskId,
      ...
      version: r.version,
      last_sync_at: r.lastSyncAt,
      // Legacy rows migrated via ALTER TABLE always get DEFAULT 1/0; the ?? guard
      // covers a hypothetical null before migration.
