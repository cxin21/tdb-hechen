  ...
  return async (c: Context): Promise<Response> => {
    ...
    if (sub === "files/download") {
      ...
      const headers: Record<string, string> = {
        ...
        // 原 serviceToken 三级兜底，鉴权翻转后 files/download 反代不再静默 401 断供。
        "Authorization": `Bearer ${resolveBearerToken(config.coreSkill.serviceToken)}`,
    ...
    const headers: Record<string, string> = {
      // I-1（P2-T11+13 复核修补）：同 files/download —— 三级 token 兜底。
      "Authorization": `Bearer ${resolveBearerToken(config.coreSkill.serviceToken)}`,
      // Prefer session-derived tenant; fall back to config for legacy sessions.
      "x-tdai-service-id": ids.space_id || config.coreSkill.serviceId,
      "Content-Type": "application/json",
