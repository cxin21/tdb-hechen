...
export class VectorStore implements IMemoryStore {
   ...
   */
  getAnchorGrowthState(tenant?: CoreTenant): { lastDiscoveryAt: string | null; lastCorpusCount: number | null } {
    try {
      const keys = this.growthStateKeys(tenant);
      const rows = this.db.prepare("SELECT k, v FROM anchor_growth_state WHERE k IN (?, ?)").all(keys.at, keys.count) as unknown as Array<{ k: string; v: string }>;
      ...
      return { lastDiscoveryAt: map.get(keys.at) ?? null, lastCorpusCount: count };
    } catch (err) {
  ...
  private growthStateKeys(tenant?: CoreTenant): { at: string; count: string } {
    if (!tenant) return { at: "last_discovery_at", count: "last_corpus_count" };
    const t = normalizeCoreTenant(tenant);
    if (t.teamId === "default" && t.userId === "default" && t.agentId === "default") {
      return { at: "last_discovery_at", count: "last_corpus_count" };
    }
    const suffix = JSON.stringify([t.teamId, t.userId, t.agentId]);
    return { at: `last_discovery_at:${suffix}`, count: `last_corpus_count:${suffix}` };
  }

  /** GROW：自生长调度状态写（发现轮次完成后调用；失败重抛——由自生长模块整体 catch）。
   *  PA：tenant 语义同 getAnchorGrowthState（default/缺省旧键，非 default per-tenant 键）。 */
  setAnchorGrowthState(state: { lastDiscoveryAt: string; lastCorpusCount: number }, tenant?: CoreTenant): void {
    try {
      const keys = this.growthStateKeys(tenant);
      this.db.prepare(
