...
export function renderTdaiMemoryToolsBlock(
  ...
  const lines: string[] = [
    ...
    "```",
    "Windows PowerShell 环境请改用（bash 单引号 JSON 内联在 PowerShell 会被吞，且 `curl` 是 Invoke-WebRequest 别名）：",
    "```powershell",
    `$body = '{"query":"用户偏好的编程语言","limit":5}'` + "  # 或使用 @{ ... } | ConvertTo-Json -Compress",
    `$tmp = Join-Path $env:TEMP ("m_{0}.json" -f [guid]::NewGuid()); Set-Content -Path $tmp -Value $body -NoNewline -Encoding utf8`,
    `curl.exe -sfk -X POST "${bridge}/atomic/search" -H "Content-Type: application/json" -H "x-conversation-id: <sessionId>" -H "x-tdai-service-id: <spaceId>" -d "@$tmp"; Remove-Item $tmp`,
    "```",
    "  要点：用 `curl.exe`（避开别名）；body 写临时文件 + `-d \"@文件\"`（避免单引号 JSON 被 PowerShell 吞）。",
    "</tdai_memory_tools>",
  ];
