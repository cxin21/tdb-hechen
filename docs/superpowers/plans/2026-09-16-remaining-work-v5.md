# 剩余工作 · 第一性原理分析文档 v5
> [已被 v6 取代] 本文件待办部分由 v6（2026-09-19-remaining-work-v6.md，REG-REMAINING-006）取代；不做项判定与 O1-O6 观察继承有效。—— 2026-09-19
...
| O15 | 灵魂注入主语分析（初判"身份污染"经用户纠正后改写）：主语须分三层——所有者主语（数据属于谁）/内容主语（说的是谁）/功能主语（为谁用）。结论：记忆=agent 的经历（所有者=agent，certainty 实见/推断=agent 认识论、significance=agent 判断、valence=agent 归档的情感评价）；identity=用户身份（内容主语=用户，拍板明确）；锚=agent 的价值参照系/行动方针（"驱动我行动的价值"我=agent 正确）。**注入框架自洽，无身份污染**；初判的"改第一人称"方案作废（那才会真污染） | 灵魂 SOP 2026-09-17 两轮质询 | 剩余实项：identity 提取 prompt（:41"这个 agent/团队是谁/我是谁"）与 identity=用户身份拍板不一致，应改"这个 agent 所服务的用户是谁/他是谁"——待拍板后一行改动（**2026-09-17 P1 已落地**：dual prompt 主语="他是谁"，gated 生效；legacy prompt 字节级保留） |
| O16 | 真灵魂缺口：agent 自我层缺失——灵魂四层中"我是谁"（职责/风格/承诺/红线）无落地层：identity 槽=用户身份、锚=源自用户价值的方针、记忆=经历、感受=方针渲染；agent 的后天品格（每周五出周报的承诺、拒绝投资建议的红线执行、先给结论的风格）只散落在经历里，无自发现/维护/注入通道。strict_rule/core_value 槽已在写入口白名单（config.ts:872）但分级门 pending 无落点（O13）——恰好是红线层的现成入口 | 真灵魂第一性分析 2026-09-17：灵魂的主语必须是 agent（无状态函数的连续性机制+责任主体），内容对象=以 agent 为中心、用户为明确标注的关系层；"换用户测试"（换用户后灵魂应剩下职责/品格）当前为零 | 择机项：双槽制（self_identity + user_identity）或 identity 槽内分段；提取 prompt 双视角；锚的品格化远方向（从 agent 行为聚合品格锚，如"严谨"）——语料含 agent 回复是前提，待查证（**2026-09-17 P1 已落地双槽与双视角注入**：self_identity 槽+agentAct 提取视角；品格聚合锚仍在远方向） |
| O17 | 记忆属性扩展盘点（方法论：属性合法性=有明确消费方（排序/渲染/维护/守卫）且不可从已有属性稳定派生）：**已覆盖**——时间（三时钟双时态：occurred_at/valid_start-end/updated_time）、场景（L1 scene_name + L2 蒸馏块）、情绪（valence/arousal 维度制）、重要性（significance）、使用度（recall_count 原子自增 P0-T4）、冲突状态（双时态+evidence_ids+staleLineage）；**不建**——类别情绪（渲染时从二维派生，避免双源漂移）、身份相关性-召回向（自我确认回音室，召回必须查询驱动）；**真缺口（按序）**——①人物实体层（记忆提及人物无结构，entity_* 表是知识图谱组织结构非记忆人物）→②关系权重=人物锚（复用 GROW 全套管道：label=人物/weight=证据强度/valence=关系情感方向，D6 派生维护量而非写时字段）→③身份维护向证据链（identity GROW-MAINT 重验证+支撑灵魂的记忆遗忘保护）；**拍板项**——敏感度/隐私级（健康/财务/关系，召回门控与遗忘优先级用，产品边界） | 属性扩展讨论 2026-09-17 | 待拍板后按序实施 |

| O18 | **per-instance store 架构**（flowtest 实证）：服务按 `x-tdai-service-id` 分实例库（`/data/tdai-memory/instances/{id}/vectors.db`），而 lifecycle 的 identity/anchor 自发现 worker 只枚举 default 实例库的记忆——非 default 实例接入方的语料不参与身份/锚自发现（flowtest：service-id=ev5-soul-team 实例库 recall 有记忆但 core_memory 恒空） | P1 flowtest 2026-09-17：conversation/add 走 ev5-soul-team 实例库、worker 走 default 库，双库数据面互不可见 | 择机项：P2 评估 per-instance 调度（每实例一轮 tick）或 worker 多实例枚举；单实例部署（当前产线形态）不受影响 |
| O19 | MemoryPanel 仓库文件为 **CRLF 行尾**：python 补丁脚本 newline 翻译会造成整文件翻转（P1 Task6 实证：4423/4064 行假 diff，8ebfabe→e80c995 已恢复） | P1 Task6 2026-09-17 | 流程约束：后续 MemoryPanel 补丁一律二进制安全（rb+wb / newline=''）；MemoryCore 为 LF 不受影响 |
| O20 | typecheck 基线 **243 行预存错误**（pipeline-factory.ts:1157 TS2322 回调方差、adapters/llm-runner 多处、api-trace 导出缺失等），P1 期间作为回归基线使用（新增零容忍，五次提交均持平） | P1 全程 2026-09-17 | 择机项：专项清理一轮（多为类型收窄/导出修正，无行为风险）；清理前禁止把 typecheck 当门禁 |
| O21 | ev5_* 测试数据留存：default 库 32 条种子 + ev5-live-s1/s2 两轮对话 L1 + 3 租户 core_memory slots + `instances/ev5-soul-team/` 残留实例库（含 3 条提取 L1） | P1 flowtest 2026-09-17 | 待授权清理（同 ev2_/ev3_/ev4_ 口径）；生产租户数据未触碰 |
