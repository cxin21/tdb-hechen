...
/**
 * 在已加载 sqlite-vec 的 db 上建 `wiki_vec` vec0 向量表（Task 3）。
 * - 表已存在且维度不同（旧 schema）→ `DROP` 重建，避免 `IF NOT EXISTS` 变成 no-op 留下旧维度；
 * - 任何失败 → 静默降级纯 FTS（不抛），调用方据此跳过向量。
 * 维度能推进建表 DDL（dimensions 无法参数化），故每次按当前维度校准。
 */
function ensureWikiVecTable(db: Database.Database, dimensions: number): void {
  try {
    ...
    if (existing && (existingDim !== String(dimensions) || !hasSha)) {
      console.log(`[wiki-vec] DROP TABLE wiki_vec 重建: dim=${existingDim}(目标=${dimensions}) has_content_sha=${hasSha}`);
    ...
    /* 任何失败（扩展未加载 / schema 异常）→ 降级纯 FTS，不建向量表 */
  }
}
 ...
 * `dimensions` 可选；>0 且 sqlite-vec 加载成功时额外建 `wiki_vec` 向量表
 * （embedding 未配置/未传/为 0 → 不建向量表，与现状一致）。
 */
export function initIndexDb(wikiDir: string, dimensions = 0): void {
  ...
  try {
    ...
    if (dimensions > 0 && loadVecExtension(db)) {
      ensureWikiVecTable(db, dimensions);
    }
    db.pragma("wal_checkpoint(TRUNCATE)");
  } finally {
    db.close();
...
/**
 * 异步写连接（非事务）：打开写连接，运行 async fn，finally checkpoint + close。
 * 用于需要事务外逐条异步写的场景——如向量落库：`writeIndex`（withWriteDb 同步事务）里
 * embed 是 async 不能进入 db.transaction，故向量在独立写连接逐条写（允许部分成功）。
 */
export async function withWriteAsync<T>(
  wikiDir: string,
  ...
  applyPragmas(db);
  loadVecExtension(db); // 预载 sqlite-vec 以便写 wiki_vec（vec0 虚拟表写需扩展）；失败静默降级
  try {
    return await fn(db);
  } finally {
...
export function getReadDb(wikiId: string, wikiDir: string): Database.Database {
  ...
  if (!db || !db.open) {
    ...
    applyPragmas(db);
    loadVecExtension(db); // 读连接预载 sqlite-vec 以便 wiki_vec 检索；失败静默（降级纯 FTS）
