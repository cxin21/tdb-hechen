...
interface SessionIdFields {
  ...
  /**
   * URL 路径侧的 agentSource —— skill-bridge 的同名接口有此字段（必填、从
   * keyId 反解）；memory-bridge 侧的 toIdFields/bindingToIdFields 目前未派生，
   * 调用点读到的运行时值为 undefined（telemetry agentSource 为可选，合法）。
   * 仅为类型诚实补此可选字段；派生逻辑属行为变更，不在类型修复批次内。
   */
  agent_source?: string;
}
