...
import { getReindexState } from "../utils/reindex-state.js";
import {
...
import { resolveApiKey, resolveV3StrictIsolation } from "../utils/env-config.js";
...
// ============================
// /health — memory 子对象（T15-B 向量健康三件套）
// ============================

/** buildMemoryHealth 的输入统计（探针/计数收集层产出）。 */
export interface MemoryHealthStats {
  /** vec0 shadow 表（l1_vec_rowids）真实向量行数；store 缺失/查询失败 → 0。 */
  vecRows: number;
  /** 元数据行数（countL1）；store 缺失/查询失败 → 0。 */
  metaRows: number;
  /** embedding 探活状态（embed-only 探针，不写哨兵行）。 */
  embeddingStatus: "ok" | "degraded";
...
/**
 * 纯函数：stats → /health 的 memory 子对象（T15-B 抽出便于直测，与 server 实例解耦）。
 */
export function buildMemoryHealth(stats: MemoryHealthStats): MemoryHealthInfo {
  return {
    ...
    degradedSince: stats.degradedSince,
    lastVecWriteAt: stats.lastVecWriteAt,
  };
}
...
export class TdaiGateway {
  ...
  // ── T15-B（向量健康三件套）：embed-only 探活 + memory 健康统计 ──
  /** embedding 探活状态；翻转时 console.warn 一次（ok→degraded / degraded→ok），不刷屏。 */
  private embeddingProbe: { status: "ok" | "degraded"; since: string | null } = { status: "ok", since: null };
  /** 探活定时器（5min 周期，unref —— 不阻止进程退出）。 */
  private embeddingProbeTimer: ReturnType<typeof setInterval> | null = null;
  /** vecRows/metaRows/lastVecWriteAt 缓存（60s，防每次 /health 打库）。 */
  private memoryHealthStatsCache: {
    at: number;
    vecRows: number;
    metaRows: number;
    lastVecWriteAt: string | null;
  ...
  /**
   * Resolved `server.apiKey` exactly as this running instance enforces it
   * (P2-T13): when config/env omitted it, `start()` generated a temporary key
   * and wrote it back into runtime config. Callers that need to authenticate
   * against THIS instance (probes, e2e tests) must read the key from here —
   * never parse it from logs or guess from ambient yaml/env — so the caller
   * key and the server key stay from the same source.
   */
  get resolvedApiKey(): string {
    return this.config.server.apiKey ?? "";
  }
  ...
  async start(): Promise<void> {
    // ── P2-T13 鉴权必填（拍板③，行为变更）────────────────────────────────
    // server.apiKey 缺失时不再放行（旧 verifyAuth 对 undefined 直接 return "ok"，
    // 门禁不设防）：启动即生成一把随机临时密钥（本进程有效，重启更换），loud 打印
    // 一次提醒运维写入配置固化。所有跨进程调用方（MemoryProxy / Panel / 探针脚本）
    // 必须配置同一把 key（env TDAI_GATEWAY_APIKEY 或各自 yaml apiKey），否则 401。
    const apiKeyResolution = resolveApiKey(this.config);
    if (apiKeyResolution.generated) {
      this.config.server.apiKey = apiKeyResolution.apiKey;
      this.logger.warn(
        `[security] server.apiKey 未配置 — 已生成临时密钥（本进程有效，重启更换；请写入配置固化）: ${apiKeyResolution.apiKey}`,
      );
    }
    ...
    // ── T15-B：启动 embedding 探活（embed-only，不写哨兵行；5min 周期 + 启动即探一次）──
    this.startEmbeddingProbe();
  }

  // ============================
  // T15-B（向量健康三件套）：embedding 探活 + memory 健康统计
  // ============================

  private startEmbeddingProbe(): void {
    const EMBED_PROBE_INTERVAL_MS = 5 * 60 * 1000;
    void this.runEmbeddingProbe(); // 启动即探一次，让 /health 尽快有真实状态
    this.embeddingProbeTimer = setInterval(() => {
      void this.runEmbeddingProbe();
    }, EMBED_PROBE_INTERVAL_MS);
    // managed timer 先例（lifecycle-scheduler / timer-scanner）：unref，不阻止进程退出
    this.embeddingProbeTimer.unref?.();
  }

  private stopEmbeddingProbe(): void {
    if (this.embeddingProbeTimer) {
      clearInterval(this.embeddingProbeTimer);
      this.embeddingProbeTimer = null;
    }
  }

  /**
   * embed 探针：只 embed 一个探针词，成功 → embedding=ok；抛错 → embedding=degraded。
   * 简化裁决（brief T15-B）：不写哨兵行（污染主表），embed-only 探活。
   * 状态翻转（ok→degraded / degraded→ok）时 warn 一次，不刷屏。
   */
  private async runEmbeddingProbe(): Promise<void> {
    const emb = this.core.getEmbeddingService();
    if (!emb) return; // 未配置 embedding → 无探针语义（health 的 embedding 恒 "ok" 探针缺省态）
    const prev = this.embeddingProbe.status;
    try {
      // S3（T15 审计 M-2）：探针词拼时间戳 —— 防 provider 侧对同文本 embedding
      // 结果做缓存，返回缓存命中导致"假探活"（服务实际降级但探针恒 ok）。
      // 探针 5min 一次，cache-bust 成本可忽略。
      await emb.embed(`tdai-vector-health-probe-${Date.now()}`);
      if (prev === "degraded") {
        this.embeddingProbe = { status: "ok", since: new Date().toISOString() };
        this.logger.warn(`${TAG} [vector-health] embedding probe recovered: degraded → ok`);
      }
    } catch (err) {
      if (prev === "ok") {
        this.embeddingProbe = { status: "degraded", since: new Date().toISOString() };
        this.logger.warn(
          `${TAG} [vector-health] embedding probe failed: ok → degraded (${err instanceof Error ? err.message : String(err)})`,
        );
      }
    }
  }
  ...
  /**
   * 收集 memory 健康统计（T15-B）。vecRows/metaRows/lastVecWriteAt 缓存 60s 防打库；
   * embedding 状态读探针内存值（实时）。
   */
  private async collectMemoryHealthStats(): Promise<MemoryHealthStats> {
    const now = Date.now();
    if (!this.memoryHealthStatsCache || now - this.memoryHealthStatsCache.at >= 60_000) {
      ...
      if (store) {
        try {
          if (typeof store.countL1VectorRows === "function") {
        ...
        if (typeof store.getLastVecWriteAt === "function") {
   ...
   *
   * Goals:
   *   1. State the post-flip auth posture accurately (M-3, P2-T11+13 复核修补):
   *      since 拍板③, start() guarantees server.apiKey is always set (a temporary
   *      key is generated and loudly printed when config omits it) — the old
   *      "auth disabled" state no longer exists, so the dead-branch copy was
   *      replaced to match reality.
   *   2. Warn when the gateway is bound to anything other than the loopback
   *      interface (cross-process callers must then hold the same key).
   *   3. Never log the key itself.
   */
  private logSecurityPosture(): void {
    const { host, apiKey, corsOrigins } = this.config.server;
    // 拍板③翻转后：start() 已保证 apiKey 恒有值（缺失时生成临时密钥并 loud
    // 打印一次），auth 恒 enforced —— 旧 "auth disabled" 分支为死分支，
    // 文案与翻转后事实对齐（不再误导运维认为存在无鉴权运行态）。
    const authEnforced = !!apiKey;

    this.logger.info(
      `Security posture: auth=${authEnforced ? "enforced (key from config or generated)" : "enforced (key from config)"} ` +
      `host=${host} cors=${corsOrigins.length === 0 ? "no-headers" : corsOrigins.includes("*") ? "wildcard(*)" : `allowlist(${corsOrigins.length})`}`
    );

    if (!authEnforced) {
      // 不可达防御分支：start() 之后 apiKey 恒有值。保留兜底文案，
      // 万一未来装配顺序变化导致空 key，也不得再打出过时的"开放访问"暗示。
      this.logger.warn(
        "server.apiKey is unexpectedly empty after start() — auth cannot be " +
        "enforced. This indicates a wiring regression; all routes except " +
        "GET /health are effectively unguarded. Fix resolveApiKey wiring.",
      );
    }
    const loopback = host === "127.0.0.1" || host === "localhost" || host === "::1";
    if (!loopback) {
      this.logger.warn(
        `Gateway is bound to ${host} (non-loopback). Cross-process callers ` +
        "(MemoryProxy / MemoryPanel / curl) must configure the same key " +
        "(env TDAI_GATEWAY_APIKEY (primary) or TDAI_GATEWAY_API_KEY (legacy), or server.apiKey in tdai-gateway.yaml) and " +
        "send `Authorization: Bearer <key>`, otherwise every call gets 401."
      );
    }
  ...
  private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    try {
      ...
      //    拍板③翻转后 server.apiKey 恒有值（缺失时 start() 已生成临时密钥），
      //    此接口恒受 Bearer 门禁保护；运维可在启动日志看到 auth posture。
      ...
      const v2Deps: V2RouterDeps = {
        ...
        // DS-RECALL-MERGE-001（合并召回）：/v3/recall 组装路径的数据根目录（与 TdaiCore
        // 的 pluginDataDir 同源——StandaloneHostAdapter 用同一 baseDir 构造）。
        getDataDir: () => this.config.data.baseDir,
        deployMode: this.config.deployMode,
        // J 重构式回忆：把 gateway config 递给 v2-router（memory.search.neighborExpand 等从这里读）
        config: this.config,
        ...
        getMetadataService: (instanceId) => this.ensureMetadataService(instanceId),
        // C2（spec §3.2）：动机方向（valence）LLM 判定 runner —— /values/derive 路由与
        // values/upsert 后 fire-and-forget 钩子共用（惰性构造 + 进程级缓存）。
        getValueValenceLlmRunner: () => this.buildValenceLlmRunner(),
      };
      ...
      const extraRoutes = {
        ...
        // /v3/admin/settings/{get,set}（spec §3.2）— 内网信任模型，管理员判定在 Panel 层。
        ...makeAdminSettingsRouteTable({
          baseDir: this.config.data.baseDir,
          getEffective: () => ({
            llm: this.config.llm as unknown as Record<string, unknown>,
            embedding: this.config.memory.embedding as unknown as Record<string, unknown>,
          }),
      ...
      // orchestrators (k8s liveness, docker health-check) rely on it.
      // S3（T15 审计 M-3）注释对齐实现：它不再是"无条件零成本"探针——60s
      // 窗口内的首次 /health 会触发 countL1VectorRows + countL1 两个 COUNT
      // 查询（collectMemoryHealthStats 缓存 60s，窗口内其余请求直接读缓存），
      // embedding 状态读内存探针值。对常规探活仍然便宜，但探活间隔应 ≥60s，
      // 高频打点才会每轮都打库。
      if (method === "GET" && pathname === "/health") {
        return this.handleHealth(res);
      }

      // All other routes go through the auth gate (P2-T13：鉴权必填 —
      // `start()` 保证 apiKey 恒有值（缺失时已生成临时密钥），无 Bearer 一律 401；
      // 唯一豁免是上面的 GET /health 探活)。
      if (!this.checkAuth(req, res)) return;
   ...
   *
   * P2-T13（拍板③，行为变更）：auth is now ALWAYS on. `start()` resolves the
   * key via {@link resolveApiKey} — when `server.apiKey` was unset, a random
   * temporary key is generated and loudly printed once — so by the time any
   * request is handled, `this.config.server.apiKey` is always non-empty and
   * requests without a matching Bearer token are rejected with 401.
   *
   * Returns one of:
   *   - `"ok"`            — token matches the (configured or generated) key
  ...
  private verifyAuth(req: http.IncomingMessage): "ok" | "missing" | "invalid" {
    ...
    if (!expected) return "ok"; // defensive: unreachable after start() (P2-T13 guarantees a key)
  ...
  private async handleHealth(res: http.ServerResponse): Promise<void> {
    // T15-B：memory 子对象（向量覆盖率 / embed 探活 / 最近向量写入）
    const memory = await this.collectMemoryHealthStats();
    const response: HealthResponse & { memory: MemoryHealthInfo } = {
      ...
      memory: buildMemoryHealth(memory),
    };
   ...
   */
  /**
   * C2（spec §3.2）：动机方向（valence）判定用 LLM runner —— 惰性构造 + 进程级缓存。
   * 构造方式与 lifecycle 块同源（resolveStandaloneLlmForRuntime + StandaloneLLMRunner）。
   * LLM 未配置 / 构造失败 → 缓存 null 哨兵并返回 undefined（R3：derive 路由 503、
   * fire-and-forget 钩子安静跳过；null 哨兵避免每个请求重复尝试构造）。
   * 返回类型与 store.deriveValueValences 的 llmRunner 参数同形（结构化最小面）。
   */
  private valenceLlmRunner?: { run(params: { prompt: string; systemPrompt?: string; taskId: string; timeoutMs?: number }): Promise<string> } | null;
  private async buildValenceLlmRunner(): Promise<{ run(params: { prompt: string; systemPrompt?: string; taskId: string; timeoutMs?: number }): Promise<string> } | undefined> {
    if (this.valenceLlmRunner !== undefined) return this.valenceLlmRunner || undefined;
    try {
      const llmCfg = (this.config as { llm?: { baseUrl?: string; apiKey?: string } }).llm;
      if (!llmCfg?.baseUrl) {
        this.valenceLlmRunner = null;
        return undefined;
      }
      ...
      const effective = resolveStandaloneLlmForRuntime(
        (this.config as { llm?: { baseUrl?: string; apiKey?: string; _provider?: unknown } }).llm as never,
        ((this as { instanceId?: string }).instanceId) ?? "default",
      );
      this.valenceLlmRunner = new StandaloneLLMRunner({
        config: {
      ...
      }) as { run(params: { prompt: string; systemPrompt?: string; taskId: string; timeoutMs?: number }): Promise<string> };
    } catch (err) {
      this.logger.warn?.("[core-memory] valence LLM runner not available: " + (err instanceof Error ? err.message : String(err)));
      this.valenceLlmRunner = null;
    }
    return this.valenceLlmRunner || undefined;
  }
  ...
  private async startIntegratedServices(): Promise<void> {
    ...
    // H+I 接线：生命周期周期调度（巩固/遗忘）。best-effort + 配置门(memory.lifecycle)。
    // consolidation 用真实 StandaloneLLMRunner（复用 skill-extractor 的 LLM 解析）；LLM 不可用时仅跑 forgetting。
    try {
      const lifecycleStore = this.core.getVectorStore();
      const { startLifecycleScheduler, DEFAULT_LIFECYCLE_CONFIG } = await import("../core/lifecycle/lifecycle-scheduler.js");
      const { StandaloneLLMRunner } = await import("../adapters/standalone/llm-runner.js");
      const { resolveStandaloneLlmForRuntime } = await import("../adapters/standalone/llm-provider-resolver.js");
      // config-first（审计修复 A1）：lifecycle/consolidation/forgetting 已进 parseConfig，
      // 这里读类型化字段（不再是 unknown cast 碰运气）。
      const lifecycle = this.config.memory.lifecycle;
      ...
      try {
        ...
        if (llmCfg?.baseUrl) {
          ...
          runner = new StandaloneLLMRunner({
            config: {
              baseUrl: effective.baseUrl,
              apiKey: effective.apiKey,
              model: effective.model ?? "default",
        ...
        this.logger.warn?.("[lifecycle] LLM runner not available for consolidation: " + (err instanceof Error ? err.message : String(err)));
        runner = undefined;
      }
    ...
    // K/B1：价值锚种子（core_values 单一源落地）——表空且有配置种子时才灌，幂等。
    try {
      const coreMemoryCfg = this.config.memory?.coreMemory;
      if (coreMemoryCfg?.seedValues?.length && this.core.getVectorStore?.()) {
        const store = this.core.getVectorStore();
        if (store?.listValues && store?.upsertValue) {
          // GROW：判空改用全态读——若仍用 active-only，全种子被 veto 后重启会误判空桶
          // 重灌种子（upsert 冲突 → state='active'），复活用户明确否决的锚（报告 §0.2）。
          const anyStore = store as typeof store & { listValuesAnyState?: () => unknown };
          const existing = anyStore.listValuesAnyState
            ? await (anyStore.listValuesAnyState as () => Promise<unknown[]>).call(store)
            : await (store.listValues as () => Promise<unknown[]>).call(store);
          if (!existing || existing.length === 0) {
            let seeded = 0;
            for (const v of coreMemoryCfg.seedValues) {
              try { (store.upsertValue as (id: string, label: string, w: number, by?: string, origin?: string) => unknown).call(store, v.id, v.label, v.weight, v.createdBy, undefined, undefined, "seed"); seeded++; }
              catch { /* best-effort */ }
            }
            this.logger.info(`[core-memory] seeded ${seeded} value anchor(s) into core_values (B1 单一源)`);
          }
        }
      }
    } catch (err) {
      this.logger.warn?.("[core-memory] value seed skipped/failed: " + (err instanceof Error ? err.message : String(err)));
    }
    ...
    try {
      ...
      this.logger.warn?.("[core-memory] boot valence derive skipped: " + (err instanceof Error ? err.message : String(err)));
    }
