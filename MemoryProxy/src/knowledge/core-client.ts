...
import { resolveBearerToken } from "../tdai/bearer-token.js";
...
export class CoreKnowledgeClient {
  ...
  async listKnowledge(teamId: string, opts: { serviceId?: string } = {}): Promise<KnowledgeItem[]> {
    ...
    // S2 缓存键碰撞收敛：JSON.stringify 数组序列化（T12 fix1 coreTenantCacheKey
    // 同模式）——单射、防分隔符歧义。前缀保留为首元素防跨键族互串
    // （list/listByIds/listAgentIds 维度数可能相同但值域不同）。
    const cacheKey = JSON.stringify(["list", teamId, effectiveServiceId]);
  ...
  private async _doListKnowledge(teamId: string, serviceId: string): Promise<KnowledgeItem[] | null> {
    ...
    const headers: Record<string, string> = {
      // I-1（P2-T11+13 复核修补）：env `TDAI_GATEWAY_APIKEY` → tdai.apiKey →
      // 原 serviceToken 三级兜底，鉴权翻转后本链路不再静默 401 断供。
      "Authorization": `Bearer ${resolveBearerToken(this.serviceToken)}`,
  ...
  async listKnowledgeByIds(
    ...
    const effectiveServiceId = opts.serviceId || this.serviceId;
    // S2：ids 以排序数组作为嵌套维度——排序保持"顺序无关命中"语义，
    // 同时消除旧 join(",") 的 ids 内部逗号歧义（["a,b"] vs ["a","b"]）。
    const sortedIds = [...ids].sort();
    const cacheKey = JSON.stringify(["listByIds", teamId, sortedIds, effectiveServiceId]);
  ...
  async listAgentKnowledgeIds(
    ...
    // S2：JSON 数组序列化防 `:` 歧义（同 listKnowledge 注）。
    const cacheKey = JSON.stringify(["listAgentIds", agentId, effectiveServiceId]);
  ...
  private async _post<T>(
    ...
    const headers: Record<string, string> = {
      // I-1（P2-T11+13 复核修补）：同 listKnowledge —— 三级 token 兜底。
      "Authorization": `Bearer ${resolveBearerToken(this.serviceToken)}`,
...
function configKey(c: Pick<CoreSkillConfig, "endpoint" | "serviceToken" | "serviceId" | "timeoutMs">): string {
  // S2：JSON 数组序列化防 `::` 歧义（T12 fix1 coreTenantCacheKey 同模式）。
  return JSON.stringify([c.endpoint, c.serviceToken, c.serviceId, c.timeoutMs]);
}
