...
/** 归档桶条目（网关 /v3/atomic/archive/list 行 shape）。SEC-1 起出参带租户三列。 */
...
/**
 * 归档可见 + 一键恢复（Task 4，DS-PANEL-UI-WIKI-SOURCE-001 §2.2）。
 *
 * L1 视图底部追加的归档区：开关打开时拉取 /chat-memory/archive/list（默认隐藏）；
 * 每条带「已归档」徽标 + 「恢复」按钮（Owner-only 由 BFF 把关）。恢复成功即从
 * 归档区移除，主时间轴下次加载时出现。
 * SEC-1 后网关 archive/list 按请求 isolation 三元组过滤（BFF idFields 透传即按
 * 当前所选 agent 过滤——前端零改动），列表只含本租户归档；旧后端出参无归属字段
 * （additive 兼容），此时不渲染归属徽标。
 */
function ArchivedSection({ blockId, visible }: { blockId: string; visible: boolean }) {
  ...
  useEffect(() => {
    ...
    chatMemoryApi
      ...
      .catch(() => {
        // 归档区是旁路视图：失败不弹全局错误，区内如实显示失败态
        if (!cancelled) setItems([]);
        tea.notify.error(t('memory.notify.archivedLoadFailed'));
      })
      .finally(() => {
  ...
  async function handleRestore(id: string) {
    setRestoringId(id);
    try {
      await chatMemoryApi.archiveRestore(blockId, id);
      setItems((prev) => prev.filter((it) => it.record_id !== id));
      tea.notify.success(t('memory.notify.restored'));
    } catch (e) {
      tea.notify.error(e instanceof Error ? e.message : t('memory.notify.restoreFailed'));
    } finally {
      setRestoringId(null);
    }
  }
  ...
  return (
    <div className="_memory-archived">
      <div className="_memory-archived-title">{t('memory.detail.archivedCount', { count: items.length })}</div>
      {loading ? (
        <div className="_memory-archived-empty">{t('memory.detail.loading')}</div>
      ) : items.length === 0 ? (
        <div className="_memory-archived-empty">{t('memory.detail.archivedEmpty')}</div>
      ) : (
        <ul className="_memory-archived-list">
          {items.map((it) => (
            <li key={it.record_id} className="_memory-archived-item">
              <div className="_memory-archived-head">
                <span className="_memory-badge _memory-badge--warning">
                  {t('memory.badge.archived')}
                </span>
                <span className="_memory-archived-record" title={it.record_id}>
                  {it.record_id}
                </span>
                <span className="_memory-archived-time" title={it.archived_at}>
                  {formatDisplayTime(it.archived_at)}
                </span>
                {it.reason && <span className="_memory-archived-reason">{it.reason}</span>}
                {/* SEC-1：归属徽标（agent 名）——出参带租户三列才渲染（旧后端 additive 兼容） */}
                {it.agent_id && (
                  <span className="_memory-archived-reason" title={t('memory.detail.archivedOwner', { agent: it.agent_id })}>
                    {t('memory.detail.archivedOwner', { agent: it.agent_id })}
                  </span>
                )}
                <Button
                  type="weak"
                  loading={restoringId === it.record_id}
                  onClick={() => void handleRestore(it.record_id)}
                >
                  {t('memory.detail.restore')}
                </Button>
              </div>
              <pre className="_memory-archived-body">{it.content}</pre>
            </li>
          ))}
        </ul>
      )}
...
/** 记忆图（G）：单条 L1 的关联记忆视图（邻接列表 + 🔗 getPath 关联链）——
 *  U-A3 迁移至 block-detail/RelatedSection（行为不变），此处仅接线。 */
...
function AtomicHead({
  ...
  loading: boolean;
  onToggle?: () => void;
  /** 记忆图（G）：所属记忆块 id，用于拉邻接；缺省不渲染相关记忆视图 */
  blockId?: string;
}) {
  const { t } = useTranslation();
  ...
  return (
    <ul className="_memory-detail-atomic-list">
      {items.map((it) => {
        ...
        // 时间轴网格：L1 按发生日期（occurred_at ?? valid_start ?? created_at）分日插日期头
        const day =
          layer === 'L1'
            ? (it.occurred_at ?? it.valid_start ?? it.created_at ?? '').slice(0, 10)
            : '';
        const showHeader = layer === 'L1' && !!day && day !== prevDay;
        prevDay = day;
        ...
        return (
          <Fragment key={it.id}>
            {showHeader && (
              <li className="_memory-detail-atomic-timeline-date" key={`${it.id}-d`}>
                {day}
              </li>
            )}
            {itemEl}
...
export function BlockDetail({
  ...
  // L1 视图模式：list(平铺=含日期分组时间轴) | graph(记忆图 canvas)
  const [l1View, setL1View] = useState<'list' | 'graph'>('list');
  // 归档可见开关（Task 4）：默认隐藏；切块时复位
    ...
    <div className="_memory-detail">
      ...
      <div className="_memory-detail-body">
        {layerLoading ? (
          ...
          <div className="_memory-l1-view">
            {layer === 'L1' && (
              <div className="_memory-view-toggle">
                <button
                  type="button"
                  className={l1View === 'list' ? '_active' : ''}
                  onClick={() => setL1View('list')}
                >
                  时间轴 / 列表
                </button>
                <button
                  type="button"
                  className={l1View === 'graph' ? '_active' : ''}
                  onClick={() => setL1View('graph')}
                >
                  记忆图
                </button>
                {/* 归档可见开关（Task 4）：默认隐藏；打开后在时间轴下方追加归档区 */}
                <button
                  type="button"
                  className={showArchived ? '_active' : ''}
                  onClick={() => setShowArchived((v) => !v)}
                >
                  {t('memory.detail.showArchived')}
                </button>
              </div>
            )}
            {layer === 'L1' && l1View === 'graph' ? (
              <MemoryGraphView blockId={block.id} items={block.layers.L1} />
            ) : (
              <AtomicList
                layer={layer}
            ...
            {layer === 'L1' && l1View === 'list' && !isSearching && (
              <ArchivedSection blockId={block.id} visible={showArchived} />
            )}
          </div>
        )}
