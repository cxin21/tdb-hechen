...
/**
 * 连接探测结果（DS-PANEL-UI-WIKI-SOURCE-001 POST /wiki/:id/test）。
 * 秒级探测，不做任何拉取/落盘。
 */
export interface SourceProbeResult {
  /** 远端是否可达。 */
  reachable: boolean;
  /** 远端分支列表（refs/heads/*，供编辑时下拉选择）。不可达时为空数组。 */
  branches: string[];
  /** 不可达时的错误摘要。 */
  error?: string;
}
...
export interface ISourceFetcher {
  ...
  /**
   * 可选：连接探测（git ls-remote 级别，秒级）。实现者内部必须先 validate()。
   * 未实现的协议（如 local/ftp）此方法缺省，调用方自行降级处理。
   */
  probe?(sourceUrl: string, branch?: string): Promise<SourceProbeResult>;
