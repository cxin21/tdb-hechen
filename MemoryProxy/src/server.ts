...
import { createCreditPricingHandlers } from "./routes/credit-pricing.js";
import { createLlmProviderHandlers } from "./routes/llm-providers.js";
import { hasAnalyseMarker, hasCostGuardMarker } from "./routes/whitelist.js";
import { getLlmProviderStore } from "./member-llm/index.js";
...
// Skill bridge: LLM curls land here, proxy injects auth + identity, forwards to core.
  ...
  // ── /v3/admin/llm-providers —— 成员自定义 LLM Provider 自服务 ─────────────
  // 鉴权 = 用户自己的 user_key(verifyUserKey,非 admin secret);
  // space_id 经查询参数传入(该管理路径无 spaceId 段,面板透传登录实例)。
  // 归属判定/scope=me 复用同一进程内 store 实例(缓存写失效即刻生效)与
  // session-init 同款 listTeams + listAgents。
  const llmProviderHandlers = createLlmProviderHandlers(config, {
    store: getLlmProviderStore(config),
    authenticate: async (c) => {
      const userKey = extractBearerToken(c.req.header("authorization") ?? "");
      const serviceId = c.req.query("space_id") ?? "";
      if (!userKey || !serviceId) return null;
      const { userId, rejected } = await verifyUserKey(userKey, serviceId);
      if (rejected || !userId) return null;
      return { userId, serviceId, userKey };
    },
    getClient: (auth) => getMetadataClient(config.coreSkill, auth.serviceId, auth.userKey),
  });
  app.get("/v3/admin/llm-providers", llmProviderHandlers.scoped);
  app.put("/v3/admin/llm-providers", llmProviderHandlers.put);
  app.delete("/v3/admin/llm-providers", llmProviderHandlers.delete);
