...
混合检索（全文 FTS + 语义向量，RRF 融合；id-only）。**非 `ready` 返回空结果（非错误）**。未配置向量时退纯关键词检索。
...
// 响应
{
  ...
  "data": {
    ...
    "status": "ready",
    "tools": [
      { "name": "search", "description": "混合检索（全文FTS+语义向量，RRF融合）wiki 页面内容", "params": { "query": { "type": "string", "required": true } } }
    ]
  }
}
```
