...
- **三维量化判据成文**（2026-09-26-task7-soul-authenticity-plan.md）：溯源率（≥95%，行级明细口径，豁免集=感受段/w 值/五徽章）、换用户/换 agent 双测试分化率（≥10 组，复用任务 2 golden 桶+B 工作流双测试）、反自我强化审计（R-A 单一源断言/F14-bis 注入不回灌/红线类唯一写通道三红线，零触发口径）。实施边界=纯测量面（gated 待拍板），行为零变更。
