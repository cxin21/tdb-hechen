...
export interface WikiBuildContext {
  ...
  ingestRunId: string;
  /** git-source 字段（source_type==='git' 时有效）；worker 据此做 git 前置阶段 */
  source_type?: string;
  source_url?: string;
  /** 分支，缺省由 worker/feating 层回退 'main' */
  branch?: string;
  path_include?: string | null;
  path_exclude?: string | null;
}

export interface WikiBuildResult {
  pageCount?: number;
  /** true 表示本次 build 因上游无变化而跳过重建（worker 短路）。 */
  skipped?: boolean;
}
...
/**
 * 读路径可用性判据（设计 006 / WAL 语义）：wiki 能否对外提供检索 / 图 / 页读取。
 * - ready：当然可读。
 * - pending / processing（定时同步/重建进行中）：只要该 wiki **曾成功同步过**
 *   （last_sync_at 已有落库值），旧索引仍在 disk、WAL 读连接不阻塞，可安全返回旧数据，
 *   避免重建期间查询被整锅置空。last_sync_at 只在成功 ready 时写入，重同步/失败期间
 *   不清空；draft（从未 ready）时必为 null，故 last_sync_at != null 足够排除全新建库。
 *   draft / failed / 首建 processing && 无 last_sync_at → 无旧索引可读，返回不可用。
 */
export function wikiCanServeReads(row: { status: string | WikiStatus; last_sync_at: string | null }): boolean {
  if (row.status === "ready") return true;
  if (row.status === "draft") return false;
  // pending / processing / failed：曾同步过才放行（有旧索引）
...
export class WikiService {
  ...
  /**
   * 源管理更新（DS-PANEL-UI-WIKI-SOURCE-001 PATCH /wiki/:id/source 的服务层落点）。
   * url/branch 的合法性校验（fetcherRegistry https-only + SSRF）与 stale 置位判定
   * 在路由层完成（与 create 的校验位置对称）；本方法只负责持久化。返回更新后行或 null。
   */
  updateSource(
    serviceId: string,
    wikiId: string,
    patch: { source_url?: string; branch?: string; enabled?: boolean; stale?: boolean },
  ): WikiRow | null {
    this.store.updateWikiStatus(serviceId, wikiId, patch);
    return this.store.getWikiById(serviceId, wikiId);
  }
  ...
  pageLs(serviceId: string, teamId: string, wikiId: string): { id: string; title: string; type: string; path: string; description?: string; locked?: boolean }[] | null {
    const row = this.store.getWiki(serviceId, teamId, wikiId);
    if (!row) return null;
    if (!wikiCanServeReads(row)) return [];
  ...
  private enqueueBuild(row: WikiRow): void {
    this.queue.enqueue(row.wiki_id, () => this.runBuild(row));
  }

  private async runBuild(row: WikiRow): Promise<void> {
    const { service_id: serviceId, wiki_id: wikiId, team_id: teamId, name } = row;
    ...
    try {
      const result = await this.worker({
        ...
        ingestRunId,
        // git-source 字段透传给 worker（worker 据此决定 git 前置阶段）
        source_type: row.source_type ?? undefined,
        source_url: row.source_url ?? undefined,
        branch: row.branch ?? undefined,
        path_include: row.path_include ?? null,
        path_exclude: row.path_exclude ?? null,
      });
      ...
      // skip（上游无变化）时保留现有 page_count，不归零；仅确认 last_sync_at 最新。
      // stale 清除（DS-PANEL-UI-WIKI-SOURCE-001）：重建成功即源信息已生效——skip 路径
      // 指纹含 branch/include/exclude，能走到这里说明索引与当前源配置一致，同样清除。
      // 失败路径不清（catch 分支不写 stale），徽标保持可见，配合 sync_error 排障。
      ...
      if (!result?.skipped) {
        readyPatch.page_count = result?.pageCount ?? null;
      }
      this.store.updateWikiStatus(serviceId, wikiId, readyPatch);
      const synced = this.store.getWikiById(serviceId, wikiId);
      if (synced) {
        const note = result?.skipped
          ? `no-change (skipped rebuild), pages: ${result?.pageCount ?? "?"}`
          : `pages: ${result?.pageCount ?? "?"}`;
        this.audit(synced, "ready", note);
      }
      this.logger?.info?.(
        result?.skipped
          ? `[wiki] ${wikiId} ready (no change, kept existing index, pages: ${result?.pageCount ?? '?'})`
          : `[wiki] ${wikiId} ready (pages: ${result?.pageCount ?? '?'})`,
      );

      // Auto-generate summary + callback TMC：仅在真正重建后执行；skip 跳过 LLM summary（避免每轮白烧 LLM），
      // 但仍发 TMC ready 回调告知状态已确认为最新。
      await this.onBuildComplete(synced, "ready", null, ingestRunId, { skipSummary: result?.skipped });
    } catch (err) {
  ...
  private async onBuildComplete(
    ...
    if (status === "ready" && !opts?.skipSummary) {
