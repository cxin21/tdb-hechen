# 剩余工作 · 第一性原理分析文档 v4（**已被 v5 取代**：REG-REMAINING-005，见 remaining-work-v5.md；本文档保留作 v4 完成项与验证轮成果的留档）
...
### 5. 提取覆盖性与批次/重试策略实证 ✅ 已修复（2026-09-16 深夜，见文末补记）
...
**触发条件**：~~现在（观测 ~1 小时级）~~ ✅ 已完成。**量级**：插桩 + 对账 ~60 行；修复视实证 0-80 行。

> **修复补记（2026-09-16 深夜）**：实证结论为**双根因**（游标未越过消息，纯触发丢失面）——
> ① 重启触发丢失：LocalStateBackend 全内存 count+timer，gateway 从不恢复 checkpoint 状态；② 续批定时器
> 被 executor 的 count===0 去重饿死（armL1IdleAfterDrain 曾复用 L1_idle 成员）。修复：boot recovery
> （checkpoint runner_states 逐会话重挂 L1_drain）+ L1_drain 独立成员 + executor isDrainTimer 豁免；
> 到期 L1 任务游标治理、无积压零成本空跑。session-g 事故完整复演闭环（重启→recovery→续批→游标收敛），
> 详见 CHANGELOG。无新开关。
...
| 优先级 | 项 | 触发 | 量级 |
|---|---|---|---|
| **1** | 提取覆盖性与游标修复（#5） | ✅ 已修复（2026-09-16 深夜） | 双根因实证 + 3 文件 ~100 行 |
| **1** | anchor-growth 门静默 debug 化（#7） | ✅ 已完成 | ~7 行 |
...
> 定时器 + executor 对 drain 任务豁免 count===0 去重；历史滞留（session-c/e 尾段）一次性清账落 L1；
> session-g 完整复演"注入→前 10 提取→重启→自愈续批→游标收敛"。#7 锚门 debug 化同步完成，重启日志
> 实见 `firstBlockReason=interval`。回归：tsc 243 持平 · vitest 472/472（+7 golden）· 四服务健康 ·
> 调参 byte-identical 还原。等门槛项（#1/#2/#3/#4/#6/#8）判定不变，勿抢跑。
