...
const WIKI_TOOLS: HttpToolDef[] = [
  ...
  {
    name: "search",
    description: "混合检索（全文 FTS + 语义向量，RRF 融合）wiki 页面内容。支持关键词与业务语义描述（如“发票结算时暂估应收怎么冲掉”）；未配置向量时自动退化为关键词检索。",
    params: {
...
async function executeWikiTool(
  serviceId: string,
  toolName: string,
  row: { wiki_id: string; team_id: string; status: string; name: string; last_sync_at: string | null },
