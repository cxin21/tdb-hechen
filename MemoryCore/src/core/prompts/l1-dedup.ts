...
3. **选择动作**：
   ...
   - "conflict"：新记忆与已有记忆**相互矛盾**（如同一事实两种说法、前后决策冲突），两者都真实存在过 → 保留双方（新记忆照常新增），用 conflict 标记矛盾关系（target_ids 填被冲突的旧记忆，不删除）。
...
export interface CandidateMatch {
  ...
  /**
   * top-1 候选（排除本批后）的真实 cosine 相似度（0..1，越大越近）。
   * 仅向量召回填充（sqlite searchL1Vector 的 score = 1 - cosine distance，
   * 与 no-dedup 路径同语义）；FTS 的 bm25 rank 与 cosine 不可比 → 不填（undefined），
   * 上游据此不附 top_candidate（P-D：拿不到真值不造假值）。
   */
  topScore?: number;
}

/**
 * 当前租户价值锚候选（C1）：dedup prompt 注入 + 解析过滤用同一清单（防幻觉 id，P-D）。
 */
export interface CoreValueCandidate {
  id: string;
  label: string;
}
 ...
 * 3. C1：当前租户价值锚候选清单（coreRefs 只能从中选；缺省/空 = 无价值锚，LLM 应回 []）
 *
 ...
 * @param valueCandidates - C1：当前租户价值锚候选（id+label，与解析过滤同源）
 */
export function formatBatchConflictPrompt(matches: CandidateMatch[], valueCandidates?: CoreValueCandidate[]): string {
  ...
  // Step 3.5（C1）：价值锚候选清单——prompt 给候选与解析过滤用同一份（防幻觉）。
  const valueSection = (() => {
    const list = (valueCandidates ?? []).filter((v) => v && typeof v.id === "string" && typeof v.label === "string" && v.label.trim());
    if (list.length === 0) {
      return `## 价值锚候选清单\n\n（空，无价值锚——所有 coreRefs 输出 []）`;
    }
    const listStr = JSON.stringify(list.map((v) => ({ label: v.label })), null, 2);
    return `## 价值锚候选清单（共 ${list.length} 个，coreRefs 只能从中选）\n\n${listStr}`;
  })();
