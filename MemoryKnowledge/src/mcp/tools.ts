...
export const MCP_TOOLS: McpToolDef[] = [
  ...
  {
    name: "wiki_search",
    description: "Search wiki pages with hybrid retrieval (full-text FTS + semantic embedding vectors, fused via reciprocal rank fusion). Query with the business meaning rather than exact keywords (e.g. \"how is estimated AR reversed on invoice settlement\"); degrades to keyword-only search when vector retrieval is unavailable. Optional graph multi-hop expansion (PRD: hop, decay, minScore) walks [[wikilink]] edges from BM25 seeds to surface graph-related pages whose body doesn't match the query directly. Each result also carries `related` (neighbour pages) and the response includes `links` (edges between results) for relationship visualisation.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        hop: {
          ...
          minimum: 0,
          maximum: 5,
          description: "Graph expansion depth. 0 = no graph expansion (default; results come from hybrid FTS+vector), >0 = walk wikilink edges from BM25 seeds.",
        },
        decay: {
          type: "number",
          minimum: 0,
