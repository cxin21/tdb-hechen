...
export interface TdaiConfig {
  ...
  memory: {
    ...
    injectL2L3: boolean;
    l1Limit: number;
    /** L1 召回"自有 + 借入"跨 agent 合并后保留条数（TdaiL1RecallInjector globalTopK）。默认 5 = 原硬编码行为不变。 */
    l1GlobalTopK: number;
    l2Limit: number;
    timeoutMs: number;
  };
...
/**
 * WikiRecallInjector 配置。
 *
 * 每轮 user.before 对当前 agent 绑定的 wiki 做召回检索并注入
 * `<tdai_recalled_wiki>` 块（test-bed，方向 B）。
 *
 * 缺省（未配置本段）时按默认参数注册（enabled 默认 true，见
 * `injection/index.ts` 的 `defaultWikiRecallConfig`）；仅显式 `enabled:false`
 * 时不注册。装配时用 `config.wikiRecall ?? defaultWikiRecallConfig` 兜底。
 */
export interface WikiRecallConfig {
  /** 总开关。`false` → 不注册；整段缺失或 `true` → 注册。默认 true。 */
  enabled: boolean;
  ...
  /** 召回相关度下限。默认 0.02。 */
  minScore: number;
  /** 图展开 hop 数；0 = 不启用（V3 决策）。默认 0。 */
  hop: number;
  /** hop decay（仅 hop>0 时随请求下发）。默认 0.5。 */
  decay: number;
  /** 注入块总字符上限（code points）；<=0 表示不限制。默认 0。 */
  maxTotalChars: number;
  /** 注入门槛：归一化后 normScore（0~1）低于此值的不注入；<=0 表示不启用。默认 0.6。 */
  minInjectNormScore?: number;
  /** 按 type 降权系数（可选）：entity/concept 在跨库并池+归一前乘系数降权。 */
  typeWeights?: Record<string, number>;
  /** 绝对相关门控阈值（可选，默认关）：对带 absScore 的候选生效；需先校准刻度。 */
  minInjectAbsScore?: number;
  /** 跨库软定域（可选）：按 wiki 领域词 boost（仅加权不删除）；无命中=现状。 */
...
export interface ProxyConfig {
  ...
  /** K/L 当前感受（appraisal）配置。缺失 → 注入器不注册感受块（零影响）。 */
  appraisal?: { enabled: boolean; firedThreshold: number; values: Array<{ id: string; label: string; weight: number }> };
  /** WikiRecallInjector 配置。缺失 → 通过 `config.knowledge` 门控启用默认参数。 */
  wikiRecall?: WikiRecallConfig;
...
export interface CreditReportConfig {
  /** 总开关。缺省 true = 逐位现状；false = 上报链整体短路（不 fetch、不写审计、不报错）。 */
  enabled?: boolean;
  /** POST endpoint URL. */
  url: string;
...
export interface RawYamlConfig {
  /** K/L 价值锚+当前感受（B1）：显式解析进 ProxyConfig.appraisal；缺省关。 */
  appraisal?: {
    enabled?: boolean;
  ...
  /** Wiki 自动召回（WikiRecallInjector）可选配置段。缺省走代码默认值。 */
  wikiRecall?: Partial<WikiRecallConfig>;
  skillRuntime?: {
    allowLlmWrite?: boolean;
  };
  auth?: {
...
export interface RequestLogEntry {
  ...
  /** Langfuse/Opik trace id — written by handlers that already created a trace. */
  traceId?: string;
}
