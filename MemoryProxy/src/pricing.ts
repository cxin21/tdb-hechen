...
/**
 * system-user 短路路径的模型门禁（spec §8 不变式）。
 *
 * 门禁迁移至转发缝后，system-user 短路（内部服务账号 → upstream.apiKey →
 * TokenHub 计费上游）在成员解析之前 return，不再经过转发缝门禁。迁移前入口
 * 门禁对内部调用者同样生效（旧注释契约："internal callers must also request
 * by modelName"），此薄封装恢复该不变式：内部服务账号不走成员覆盖，
 * `memberUpstream` 恒 `null`，语义与 `enforceModelGate(config, null, requested)`
 * 逐字等价。
 *
 * 返回 null=放行；string=400 错误消息（各 handler 按自身信封返回 400）。
 */
export function gateForSystemUser(
  config: CreditPricingConfig | null | undefined,
  requested: string | null | undefined,
): string | null {
  return enforceModelGate(config, null, requested);
}
