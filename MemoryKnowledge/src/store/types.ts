...
export interface WikiRow {
  ...
  last_sync_at: string | null;
  /**
   * 源启用开关（DS-PANEL-UI-WIKI-SOURCE-001 §1.2 暂停/启用）。
   * 默认 true（存量行迁移列默认 1）；false = 暂停，auto-sync 跳过该源，已摄入页面不受影响。
   */
  enabled: boolean;
  /**
   * 源信息变更标记：url/branch 实际变更时置位（PATCH source 路由），
   * 重建成功（runBuild → ready）清除；失败保持置位（徽标不静默消失）。
   */
  stale: boolean;
  created_at: string;
  updated_at: string;
...
export interface WikiStatusPatch {
  ...
  service_url?: string | null;
  summary?: string | null;
  // git-source config (set at create; carried through status flow when editor updates them)
  source_url?: string | null;
  branch?: string | null;
  path_include?: string | null;
  path_exclude?: string | null;
  // source management (DS-PANEL-UI-WIKI-SOURCE-001): enabled toggle + stale marker
  enabled?: boolean;
  stale?: boolean;
  version?: number;
}
