...
import { evolveCandidate, type DurativeInput } from "../lifecycle/persona/persona-evolution.js";
...
export class PersonaGenerator {
  ...
  private durativesProvider: (() => Promise<DurativeInput[]>) | undefined;
  ...
  constructor(opts: {
    ...
    /** M persona 演进：提供近期持续态，供反漂移筛选后并入画像更新（缺省不启用，零影响）。 */
    duratives?: () => Promise<DurativeInput[]>;
  }) {
    ...
    this.durativesProvider = opts.duratives;
  ...
  async generateLocalPersona(triggerReason?: string): Promise<boolean> {
    ...
    // M persona 演进：反漂移并入近期持续态结论（仅 observed+significant+minObserved；best-effort；缺省 provider 则零影响）
    if (this.durativesProvider && existingPersona) {
      try {
        const dur = await this.durativesProvider();
        const { proposed } = evolveCandidate(dur, existingPersona);
        if (proposed.length > 0) {
          changedScenesContent += `\n\n**新近巩固结论（已过反漂移筛选，供整合进画像参考，勿与既有事实冲突）**：\n${proposed.map((p) => `- ${p}`).join("\n")}\n`;
          this.logger?.debug?.(`${TAG} persona evolve: folded ${proposed.length} proposed conclusion(s)`);
        }
      } catch (err) {
        this.logger?.warn?.(`${TAG} persona evolve skipped (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
      }
    }
    ...
    // M 演化留痕（设计 persona §4）：启用 duratives(M) 时记录本轮画像演化痕迹
    const evoTrace = this.durativesProvider
      ? `\n<!-- persona-evolution ${new Date().toISOString()} reason=${encodeURIComponent(triggerReason ?? "")} durativesInput=true -->\n`
      : "";
    const finalContent = nav
      ? `${personaText}${evoTrace}\n\n${nav}\n`
      : `${personaText}${evoTrace}\n`;
    if (this.storage) {
