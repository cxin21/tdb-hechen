...
> **🔐 网关鉴权必填（P2-T13 起，行为变更）**
>
> Memory Core 网关不再"无 key 放行"：`server.apiKey`（或 env `TDAI_GATEWAY_API_KEY`）
> 不配置时，网关每次启动会**生成一把随机临时密钥**（本进程有效，重启更换）并在启动日志
> loud 打印一次。所有跨进程调用方必须在 env/yaml 配置**同一把 key**（env
> `TDAI_GATEWAY_APIKEY`，或各自配置文件里的 `apiKey` / `api_key` 字段），否则一律 401：
>
> - **MemoryProxy**：`TDAI_GATEWAY_APIKEY` env，或 `config.yaml` 的 `tdai.apiKey`
> - **MemoryPanel**：`TDAI_GATEWAY_APIKEY` env（覆盖全部实例），或 `config/metadata-instances.json` 每实例的 `api_key`
> - **curl / 探针脚本**：请求头加 `Authorization: Bearer <key>`（`GET /health` 豁免）
>
> 生产部署请把固定 key 写入网关 `tdai-gateway.yaml` 的 `server.apiKey`，重启不再变化。
