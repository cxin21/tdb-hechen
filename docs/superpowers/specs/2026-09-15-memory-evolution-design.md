...
**五条红线（全程不变式）**：
1. golden 验收线 P@5 ≥ 0.325 不放松；排序语义变更一律预注册 A/B（DS-P5-REANCHOR-001 §4.1）
...
### 6.1 LongMemEval 五能力 → golden harness
| 能力 | 现状 | 动作 |
|---|---|---|
| 信息抽取 | 已覆盖 | 不动 |
| 多会话推理 | 语料单会话倾向 | golden 补跨 session query |
| 时间推理 | 语料时间戳集中 | fixture 生成器：合成历史散布 occurred_at 语料 |
| 知识更新 | 无冲突语料对 | 造"A→A 被推翻"语料对（兼作 P2 验收数据） |
| 弃答 | P@5 不测 | 加 abstention 指标（无关 query 应空/低分） |
