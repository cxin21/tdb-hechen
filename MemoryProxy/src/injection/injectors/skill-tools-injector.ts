...
export function renderSkillToolsBlock(
  ...
  return [
    ...
    `  其中 <bridge> = ${bridge}`,
    "",
"Windows PowerShell 环境请改用（PowerShell 里 curl 是 Invoke-WebRequest 别名，且内联单引号 JSON 会被吞）：",
    "  $body = @{ ...业务字段... } | ConvertTo-Json -Compress",
    "  $tmp = Join-Path $env:TEMP (\"b_{0}.json\" -f [guid]::NewGuid()); Set-Content -Path $tmp -Value $body -NoNewline -Encoding utf8",
    '  curl.exe -sSk -X POST <bridge>/<action> -H "Content-Type: application/json" -H "x-conversation-id: <sessionId>" -H "x-tdai-service-id: <spaceId>" -d "@$tmp"; Remove-Item $tmp',
    "  说明：若 curl 报 `Cannot bind parameter 'Headers'` 或 body 被吞/`invalid JSON`，即未走 PowerShell 分支——务必 curl.exe + 写临时文件 + -d \"@文件\"。",
    "",
    "可用工具：",
    "",
    ...readTools,
