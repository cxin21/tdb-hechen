...
// upstream.agents[agentFromPath] > 全局。缓存为 per-module 单例(共享底层
...
// ── Session key: prefer conversation header, fallback to agent profile ───────────
  ...
  // agent 成员覆盖 > user 成员覆盖 > upstream.agents[agentFromPath] > 全局。
  // 命中成员覆盖时:转发 url / apiKey(x-api-key) / body.model 全取成员配置;
  // 未命中返回 null,下方所有解析与现状逐字一致。
