...
export interface WikiDetail {
  ...
  created_at: string;
  updated_at: string;
  // Git 源字段（spec §5.4；/wiki/create 可写入，/wiki/get 返回）
  source_type?: string | null;
  source_url?: string | null;
  branch?: string | null;
  path_include?: string | null;
  path_exclude?: string | null;
  // 源管理（DS-PANEL-UI-WIKI-SOURCE-001 §1.3）：enabled=暂停/启用；stale=源变更待重拉
  enabled?: boolean | null;
  stale?: boolean | null;
}

/** Git 源 wiki 创建入参（对齐 KS /v3/wiki/create）。 */
export interface WikiCreateOptions {
  source_type?: string;
  source_url?: string;
  branch?: string;
  path_include?: string;
...
/** 源编辑补丁（KS PATCH /wiki/:id/source）：至少一项，由 Panel 路由层校验。 */
export interface WikiSourceUpdatePatch {
  source_url?: string;
  branch?: string;
  enabled?: boolean;
}

/** 连接探测结果（KS POST /wiki/:id/test）：探测网络失败时 reachable=false + error，仍是 200。 */
...
/** refetch 接受结果（KS 202）：重建异步进行，stale 由后端在重建成功时清除。 */
export interface WikiRefetchResult {
  wiki_id: string;
  status: string;
}
...
export interface KnowledgeClientPort {
  ...
  wikiCreate(teamId: string, name: string, userId?: string, opts?: WikiCreateOptions): Promise<WikiDetail>;
  wikiGet(wikiId: string): Promise<WikiDetail>;
  wikiIngest(wikiId: string): Promise<WikiIngestResult>;
  wikiSync(wikiId: string): Promise<WikiIngestResult>;
  ...
  // Wiki — 源管理（DS-PANEL-UI-WIKI-SOURCE-001 §1.3；stale 清除时机 = 重建成功，后端负责）
  wikiUpdateSource(wikiId: string, patch: WikiSourceUpdatePatch): Promise<WikiDetail>;
  wikiTestSource(wikiId: string, sourceUrl?: string): Promise<WikiSourceProbeResult>;
  wikiRefetch(wikiId: string): Promise<WikiRefetchResult>;
