...
import type { WikiService, IngestResult } from "./wiki-service.js";
import type { IKnowledgeStore, CodeGraphRow, WikiRow } from "./types.js";
...
export function resolveAutoSyncConfig(env: Record<string, string | undefined> = process.env): AutoSyncConfig {
  ...
  const scanIntervalMin = clamp(parseFloat(env.KNOWLEDGE_AUTO_SYNC_SCAN_INTERVAL_MIN ?? "") || 10, 1, 1440);
...
export interface AutoSyncSchedulerDeps {
  ...
  /** 可选：注入后 git 源 wiki 才参与自动同步（方向 A 全局开关，无 per-wiki 配置）。 */
  wikiService?: WikiService;
  config: AutoSyncConfig;
}

/** 自动同步候选：code-graph 或 git 源 wiki 的判别联合。 */
export type AutoSyncCandidate =
  | { kind: "codegraph"; row: CodeGraphRow }
  | { kind: "wiki"; row: WikiRow };

function candidateId(c: AutoSyncCandidate): string {
  return c.kind === "wiki" ? c.row.wiki_id : c.row.code_graph_id;
}

function candidateLabel(c: AutoSyncCandidate): string {
  if (c.kind === "wiki") return `wiki ${c.row.wiki_id} (${c.row.source_url ?? "?"})`;
  return `codegraph ${c.row.code_graph_id} (${c.row.repo_url}@${c.row.branch})`;
}
...
export class AutoSyncScheduler {
  ...
  private readonly queue: AutoSyncCandidate[] = [];
  ...
  constructor(deps: AutoSyncSchedulerDeps) {
    ...
    this.wikiService = deps.wikiService;
  ...
  /**
   * 一轮扫描：
   *   1. 列出所有 ready 状态的 code-graph + 所有 ready 状态的 git 源 wiki
  ...
  private async scan(): Promise<void> {
    ...
    try {
      ...
      for (const cand of candidates) {
        ...
        if (this.inFlight.has(id)) continue; // 已在队列或处理中
        this.inFlight.add(id);
        this.queue.push(cand);
        enqueued++;
      }
      log.info(`[auto-sync] enqueued ${enqueued} candidate(s) (queue=${this.queue.length}, active=${this.activeSyncs})`);
    } catch (err) {
  ...
  /**
   * 列出需要同步的候选：
   *   - code-graph：现有逻辑（仅 status = ready 的仓库）
   *   - git 源 wiki：status = ready 且 source_type === 'git'（方向 A 全局纳入，无 per-wiki 勾选）
   * 已在队列或 worker 处理中的由 scan() 的 inFlight Set 去重；wikiService 未注入则 git wiki 不参与。
   */
  private listSyncCandidates(): AutoSyncCandidate[] {
    const candidates: AutoSyncCandidate[] = [];

    // code-graph candidates
    ...
    for (const ref of syncedRefs) {
      try {
        ...
        candidates.push({ kind: "codegraph", row });
      } catch (err) {
        log.warn(`[auto-sync] failed to check code-graph ${ref.code_graph_id}: ${err instanceof Error ? err.message : String(err)}`);
      }
    }

    // git-source wiki candidates (only when wikiService available)
    if (this.wikiService) {
      const syncedWikis = this.store.listSyncedWikis();
      for (const ref of syncedWikis) {
        try {
          const row = this.store.getWiki(ref.service_id, ref.team_id, ref.wiki_id);
          if (!row) continue;
          if (row.status !== "ready" || row.source_type !== "git") continue;
          // 源暂停（enabled=false）时跳过（DS-PANEL-UI-WIKI-SOURCE-001 §1.2）；
          // 已摄入页面不受影响，仍可正常读取。
          if (!row.enabled) continue;
          candidates.push({ kind: "wiki", row });
        } catch (err) {
          log.warn(`[auto-sync] failed to check wiki ${ref.wiki_id}: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }
  ...
  private async runWorker(workerIdx: number): Promise<void> {
    ...
    while (!this.stopped) {
      ...
      this.activeSyncs++;
      try {
        await this.syncOne(cand);
      } catch (err) {
        // syncOne 内已捕获，这里是兜底
        log.error(`[auto-sync] worker#${workerIdx} unexpected error: ${err instanceof Error ? err.message : String(err)}`);
      } finally {
        this.activeSyncs--;
        this.inFlight.delete(candidateId(cand));
      }
    }
    log.debug(`[auto-sync] worker#${workerIdx} exiting`);
  }

  /** 对单个候选执行同步：code-graph → cgService.sync；git wiki → wikiService.sync。 */
  private async syncOne(candidate: AutoSyncCandidate): Promise<void> {
    const startMs = Date.now();
    log.info(`[auto-sync] sync ${candidateLabel(candidate)}`);
    try {
      let result: SyncResult | IngestResult | null = null;
      if (candidate.kind === "codegraph") {
        result = await Promise.resolve(
          this.cgService.sync(candidate.row.service_id, candidate.row.team_id, candidate.row.code_graph_id, undefined),
        );
      } else {
        // wikiService 在 listSyncCandidates 中已保证存在（未注入则不产生 wiki 候选）
        const svc = this.wikiService!;
        result = svc.sync(candidate.row.service_id, candidate.row.team_id, candidate.row.wiki_id);
      }
      const durationMs = Date.now() - startMs;
      if (result === null) return;
      switch (result.kind) {
        case "ok":
          log.info(`[auto-sync] sync enqueued for ${candidateId(candidate)} (took ${durationMs}ms)`);
          break;
        case "busy":
          log.debug(`[auto-sync] skip ${candidateId(candidate)}: already ${result.status} (step: ${result.step})`);
          break;
        case "not_found":
          log.warn(`[auto-sync] skip ${candidateId(candidate)}: not found (may have been deleted)`);
          break;
      }
    } catch (err) {
      log.error(`[auto-sync] sync failed for ${candidateId(candidate)}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
