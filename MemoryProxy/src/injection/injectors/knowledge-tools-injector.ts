...
export function renderKnowledgeToolsBlock(
  ...
  return [
    ...
    "",
    "Windows PowerShell 环境请改用（bash 单引号 JSON 内联会被吞，且 `curl` 是 Invoke-WebRequest 别名）：",
    "```powershell",
    "$body = @{ knowledge_id = '<知识id>'; tool_name = '<Step1返回的name>'; params = @{} } | ConvertTo-Json -Compress",
    '$tmp = Join-Path $env:TEMP ("kb_{0}.json" -f [guid]::NewGuid()); Set-Content -Path $tmp -Value $body -NoNewline -Encoding utf8',
    'curl.exe -sSk -X POST "<url>/tools/call" -H "Content-Type: application/json" -H "x-tdai-service-id: <serviceId>" -H "x-conversation-id: <conversationId>" -d "@$tmp"; Remove-Item $tmp',
    "```",
    "  要点：用 `curl.exe` 避开别名；body 写临时文件 + `-d \"@文件\"`，避免 PowerShell 吞掉内联 JSON 引号导致 500 `Expected property name or '}'`。",
    "",
    "## 约定",
