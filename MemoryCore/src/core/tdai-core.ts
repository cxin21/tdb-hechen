...
export class TdaiCore {
  ...
  async searchMemories(params: MemorySearchParams): Promise<{ text: string; total: number; strategy: string }> {
    const result = await executeMemorySearch({
      ...
      embeddingService: this.embeddingService,
      logger: this.logger,
      // C1（灵魂记忆 spec §2.2）：coreRef 排序加成配置透传（缺省由 executeMemorySearch
      // 内部回落 0.05）；价值锚由 executeMemorySearch 从 store.listValues 租户读取
      coreRefBoost: this.cfg?.recall?.coreRefBoost,
      // R-A1（结构感知召回 spec §2）：排序层结构信号六开关透传（缺省由
      // executeMemorySearch 内部回落 spec §2 默认值；0=通道完全退出）
      timeBoost: this.cfg?.recall?.timeBoost,
      recencyBoost: this.cfg?.recall?.recencyBoost,
      sigWeight: this.cfg?.recall?.sigWeight,
      inferredPenalty: this.cfg?.recall?.inferredPenalty,
      reinforcementWeight: this.cfg?.recall?.reinforcementWeight,
      moodBoost: this.cfg?.recall?.moodBoost,
      // R-A2（结构感知召回 spec §2 R4/R6）：候选池通道开关透传（缺省回落 spec §2 默认值；
      // graphDiscount=0 = 图通道完全退出——spec §3 失效即关在工具路径同样成立）
      // V2-1：PPR 三旋钮透传（缺省 undefined → executeMemorySearch 回落默认——
      // 若缺此透传，config 覆写会被工具路静默忽略，登记的计划外接线）
      graphMinStrength: this.cfg?.recall?.graphMinStrength,
      graphDiscount: this.cfg?.recall?.graphDiscount,
      pprDamping: this.cfg?.recall?.pprDamping,
      pprTopK: this.cfg?.recall?.pprTopK,
      pprIterations: this.cfg?.recall?.pprIterations,
      sceneBoost: this.cfg?.recall?.sceneBoost,
      // V2 批 2（I2 修补）：引擎三 exploreSlot/rerankWeights + 引擎一 queryExpansion config
      // 透传——关断矩阵在工具路成立（缺此透传 config 覆写会被工具路静默忽略）。
      // queryExpansion 的 runner 由 executeMemorySearch 经 resolveQueryExpansionRunner(llm)
      // 惰性构造（与 auto-recall 路 cfg.llm 同源；apiKey 空 → 不构造，零外呼——C1）。
      exploreSlot: this.cfg?.recall?.exploreSlot,
      rerankWeights: this.cfg?.recall?.rerankWeights,
      queryExpansion: this.cfg?.recall?.queryExpansion
        ? { ...this.cfg.recall.queryExpansion, llm: this.cfg?.llm }
        : undefined,
    });
