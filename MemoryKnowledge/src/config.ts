...
import { mergeOverride, readSettingsOverride } from "./config-override.js";
...
/**
 * Wiki embedding 配置（知识库向量化）。复用记忆侧火山 doubao 服务。
 * 未配置（baseUrl/apiKey/model 为空或 dimensions<=0）→ getEmbeddingConfig() 返回 null，不启用向量。
 */
...
export function loadConfig(): ServiceConfig {
  ...
  // Admin settings override（{dataDir}/config-override.json）优先级高于 env。见 spec §3.1。
