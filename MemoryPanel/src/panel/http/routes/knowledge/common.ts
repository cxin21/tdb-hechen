...
export interface KnowledgeAssetListItem {
  ...
  // Git 源字段（wiki/CG 列表需透传给前端以显示 git 标签 / 同步按钮）
  source_type?: string | null;
  source_url?: string | null;
  branch?: string | null;
  path_include?: string | null;
  path_exclude?: string | null;
  // 源管理（DS-PANEL-UI-WIKI-SOURCE-001）：暂停/启用 + 待重拉徽标
...
async function joinWikiKs(
  ...
  try {
    ...
    return {
      ...
      ks_missing: false,
      source_type: ks.source_type ?? null,
      source_url: ks.source_url ?? null,
      branch: ks.branch ?? null,
      path_include: ks.path_include ?? null,
      path_exclude: ks.path_exclude ?? null,
      enabled: ks.enabled ?? null,
      stale: ks.stale ?? null,
