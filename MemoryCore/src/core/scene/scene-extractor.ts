...
import type { MemoryPromptMode, SceneGovernanceConfig } from "../../config.js";
...
export interface SceneExtractorOptions {
  ...
  /**
   * 场景块治理配置（DS-SCENE-GOV-001）。来自 config 解析链（恒解析非 optional），
   * 门控用 `.enabled`（缺省 false = 生产行为零变化）。
   */
...
export class SceneExtractor {
  ...
  private sceneGovernance: SceneGovernanceConfig | undefined;
  ...
  constructor(opts: SceneExtractorOptions) {
    ...
    this.sceneGovernance = opts.sceneGovernance;
  ...
  async extract(memories: Array<{ content: string; created_at: string; id?: string }>): Promise<ExtractionResult> {
    ...
    // Phase 5c: Scene block governance (DS-SCENE-GOV-001, Task GOV-3).
    //
    // Runs after filename normalization (canonical names reach the governor)
    // and before syncSceneIndex (the index sees governed content). Gated on
    // `sceneGovernance.enabled` (default false — bit-identical no-op in
    // production). Failure-safe: governance errors are logged loud and never
    // block the extract main flow; SceneGovernor additionally keeps per-block
    // failures non-fatal (original block preserved + report.errors).
    if (this.sceneGovernance?.enabled) {
      const govStartMs = Date.now();
      try {
        const governor = new SceneGovernor({
          // LLMRunner → 治理 runner 适配：LLMRunParams 必填 taskId（追踪语义），
          // 治理侧最小三字段接口由此闭包补齐，GOV-2/GOV-1 签名零改动。
          runner: {
            run: (p) => this.runner.run({ ...p, taskId: `scene-govern-${Date.now()}` }),
          },
          logger: this.logger,
        });
        const govReport = await governor.govern({
          dataDir: this.dataDir,
          storage: this.storage,
          config: this.sceneGovernance,
        });
        this.logger?.info(
          `${TAG} extract() scene governance: scanned=${govReport.scanned}, distilled=${govReport.distilled.length}, hardCapped=${govReport.hardCapped.length}, skipped=${govReport.skipped.length}, errors=${govReport.errors.length} (${Date.now() - govStartMs}ms)`,
        );
      } catch (govErr) {
        // 治理异常绝不外抛阻断 extract 主流程：loud warn + 照常继续索引同步。
        this.logger?.warn(`${TAG} extract() scene governance error (non-fatal, extract continues): ${govErr instanceof Error ? govErr.message : String(govErr)}`);
      }
    }
