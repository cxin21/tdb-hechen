...
// spec §3.1: 成员自定义 Provider 覆盖 url/apiKey/model。优先级 agent > user >
// upstream.agents.workbuddy > 全局。缓存为 per-module 单例(共享底层
// ProxyStorage);路由写后跨模块显式 invalidate 不可达,成员改配置最迟按
// spec §3.3 ≤30s TTL 收敛(已接受的取舍)。
...
async function forwardToUpstream(
  ...
  memberUpstream: MemberUpstream | null = null,
): Promise<Response> {
  // Member-custom provider override (spec §3.1):命中成员覆盖时转发目标
  // url/apiKey/body.model 全取成员配置,优先级高于 upstream.agents.workbuddy。
  ...
  const upstreamBase = ((memberUpstream?.url ?? perAgent?.url ?? config.upstream.url ?? "") as string).replace(/\/$/, "");
  ...
  if (memberUpstream) {
    headers["authorization"] = `Bearer ${memberUpstream.apiKey}`;
    delete headers["x-api-key"];
  } else if (perAgent?.apiKey) {
...
export async function handleWorkbuddyEndpoint(
  ...
  return forwardToUpstream(c, config, body, traceId, startTime, keyId, modelId, pipe, lf, archiveCtx, memberUpstream);
}
